	...
	if($curProductCode != '') {
	      ...
	      $query = 	"INSERT INTO 
						...
						VALUES(
							...
							'".mysqli_real_escape_string($conn,$_POST['stock_to_id'])."',		
							'".mysqli_real_escape_string($conn,$curProductCode)."',	
							'".mysqli_real_escape_string($conn,$curCount)."',	
							'".mysqli_real_escape_string($conn,$status)."',	
							NOW(),	
                            '".mysqli_real_escape_string($conn,$_POST['stock_from_id'])."',	
							...
							'".mysqli_real_escape_string($conn,$curCount)."',
                            '".mysqli_real_escape_string($conn,$_POST['requestor_name'])."',
                            NOW(),
                            '".mysqli_real_escape_string($conn,$curCount)."',
                            '".mysqli_real_escape_string($conn,$_POST['requestor_name'])."'
                        )";
...
if($_SESSION['store_code']=='warehouse'){
	echo "<script> window.location='/inventory/warehouse/history/?ref_num=".$deliveryID."'; </script>";
}else{
	echo "<script> window.location='/inventory/admin/history-all/?ref_num=".$deliveryID."'; </script>";
}
