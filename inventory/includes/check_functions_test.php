    ...
    $datenow=date('Y-m-d');
     ...
     $query ="SELECT 
                        ...
                        SELECT COALESCE( max(iaccc.date_end),0)
                            ...
                            AND
                                DATE(DATE_ADD(iib.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx.status_date), INTERVAL 13 HOUR)) FROM inventory_test iacx 
                                                                        WHERE iacx.product_code='".$product_code."'
                            ...
                            AND
                            DATE(DATE_ADD(iib.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx2.status_date), INTERVAL 13 HOUR)) FROM inventory_test iacx2 
                        ...
                        (select  DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) FROM inventory_test iid
                        WHERE
                        iid.product_code ='".$product_code."'
                        AND
                        ...
                        AND iid.type='damage'
                        AND
                        DATE(DATE_ADD(iid.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx.status_date), INTERVAL 13 HOUR)) FROM inventory_test iacx 
                                                                    WHERE iacx.product_code='".$product_code."'
                        ...
                        (select  DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) FROM inventory_test  iip
                        WHERE
                        iip.product_code ='".$product_code."'
                        AND
                        ...
                        AND iip.type='pullout'
                        AND
                        DATE(DATE_ADD(iip.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx.status_date), INTERVAL 13 HOUR)) FROM inventory_test iacx 
                                                                    WHERE iacx.product_code='".$product_code."'
                                                                AND iacx.stock_from='".$store_id."'
                                                                AND  DATE(DATE_ADD(iacx.status_date, INTERVAL 13 HOUR))<'".$date_start."' 
                                                                AND count !='0'
                        ...
                        AND
                        DATE(DATE_ADD(iib.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx3.status_date), INTERVAL 13 HOUR)) FROM inventory_test iacx3 
                        ...
                        ( select DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) FROM inventory_test  iiiboc
                        WHERE
                        iiiboc.product_code ='".$product_code."'
                        AND
                        iiiboc.stock_from=sls.store_id
                        ...
                        FROM poll_51 p51
                        LEFT JOin inventory_test ip on ip.product_code='".$product_code."'
            ...
            if(in_array($arrInvLook[0]['product_code'],$arrActualX3)){
                ...
                if(  ($arrInvLook[0]["audit_date"] >=$arrInvLook[0]["stock_transfer_status_date"] 
                                                            ...
                                                            $beg_inventory=$arrInvLook[0]["past_variance_2"] -$arrInvLook[0]["sales_deduct_physical"];
