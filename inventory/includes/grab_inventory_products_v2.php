...
// }
	...
	$arrFrames = array();
			$query =   "SELECT 
						TRIM(LEFT(p51.item_name , LOCATE(' ', p51.item_name) - 1)) AS 'product_style',
						REPLACE(p51.item_name,  TRIM(LEFT(p51.item_name , LOCATE(' ', p51.item_name) - 1)), '') AS 'product_color',
						p51.product_code,
								(SELECT 
									coalesce(
											sum(
								...
								(select coalesce(
									...
									(
																...
																SELECT COALESCE(sum(iaccc.input_count),0)- COALESCE(sum(iaccc.running),0)
																	...
																	where iaccc.date_end<'".$dateStart."'  and iaccc.store_audited='".$store_id."'
																	and iaccc.product_code=p51.product_code
																	and iaccc.date_end=(SELECT max(iacx.date_end) FROM inventory_actual_count iacx WHERE iacx.product_code=iaccc.product_code 
    ...
    // SELECT COALESCE(sum(input_count),0) FROM inventory_actual_count 
