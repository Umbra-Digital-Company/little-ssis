...
if(isset($_GET['search']) && $_GET['search'] !='' && !empty($arrSearch)) {
	$querySearch .= " AND (sl.store_name LIKE '%".$_GET['search']."%' OR ll.lab_name LIKE '%".$_GET['search']."%'    

                        OR
                        (
                            i.store_audited like '%".$_GET['search']."%'
                        )
                    )";
	...
	// 		$querySearch .= " OR (sl.store_name LIKE '%".$arrSearch[0]."%' AND sl.store_name LIKE '%".$arrSearch[1]."%') OR
	// 							(ll.lab_name LIKE '%".$arrSearch[0]."%' AND ll.lab_name LIKE '%".$arrSearch[1]."%') ";
	// }
};


if  (  (isset($_GET['date_from']) && $_GET['date_from'] !='' ) &&  (isset($_GET['date_to']) && $_GET['date_to'] !='' ) ){
    $queryDateSearch= "  AND date(DATE_ADD(i.date_created, INTERVAL 13 HOUR)) >= '".$_GET['date_from']."'  
                                and   date(DATE_ADD(i.date_created, INTERVAL 13 HOUR)) <= '".$_GET['date_to']."'
...
elseif( (isset($_GET['date_from']) && $_GET['date_from'] =='' ) &&  (isset($_GET['date_to']) && $_GET['date_to'] =='' ) && (isset($_GET['search']) && $_GET['search'] !='' && !empty($arrSearch))  ) {
    $queryDateSearch= " ";
}else{

    $queryDateSearch= "  AND month(DATE_ADD(i.date_created, INTERVAL 13 HOUR)) = '".date('m')."' 
							AND year(DATE_ADD(i.date_created, INTERVAL 13 HOUR)) = '".date('Y')."' ";
