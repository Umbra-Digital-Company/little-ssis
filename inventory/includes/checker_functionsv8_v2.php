                        ...
                        elseif($arrInvLook[0]["audit_date"] >=$arrInvLook[0]["interbranch_out_past_date"] 
                                   ...
                                   elseif( ( $arrInvLook[0]["audit_date"] <=$arrInvLook[0]["stock_transfer_status_date"]) 
                                    ...
                                    // echo "cccc";
                                                    ...
                                                    if($arrInvLook[0]["audit_date"] <=$arrInvLook[0]["damage_in_past_date"]){
                                                    $damage_in_past =  $arrInvLook[0]["damage_in_past"];
...
function StoreChecker_auditor($product_code,$store_id,$date_start,$date_end){
    ...
    if($store_id=='787'  || $store_id=='788'){
        ...
        elseif($store_id=='151'){
            ...
            AND (
                                  os.lens_option='without prescription'
                                    OR
                                    os.lens_code IN (
                                        'L001','L002','L045', 'L014', 'L015', 'L016', 'L017', 'L018', 'L020', 'L021', 'L022', 'L023', 'L024', 'L029', 'L030', 'L031', 'L032',
        ...
        AND (
                         os.lens_option='without prescription'
                        OR
                        os.lens_code IN ('L013', 'L014', 'L015', 'L016', 'L017', 'L018', 'L020', 'L021', 'L022', 'L023', 'L024', 'L029', 'L030',
                         'L031', 'L032', 'L033','SO1001', 'L049', 'L050', 'L051', 'L052', 'L053','SO001' ,'SO002','SO003' 
        ...
        || preg_match("/DMP/i", $product_code)  || preg_match("/DD/i", $product_code) || preg_match("/HC/i", $product_code)    || preg_match("/AFC/i", $product_code)   || preg_match("/P1/i",$product_code)
        || preg_match("/MSCL/i",$product_code)   || preg_match("/SBD/i",$product_code)  || preg_match("/SDB/i",$product_code)  || preg_match("/SSWP/i",$product_code)   || preg_match("/SWB/i",$product_code)  
     ...
     $query ="SELECT 
                        ...
                        ( SELECT if( os.packaging_for!='' && packaging_stock!='store',
                                        ...
                                        count(po_number))
                                                   ...
                                                   ". $carekits."  ='".$product_code."'
                                                    ".$reRoute." 
                                                    AND  (( o.origin_branch=sls.store_id   and dispatch_type!='packaging'  )    OR   ( o.store_id=sls.store_id   and dispatch_type='packaging')   )
                                                )  as sales,
                        sls.phone_number,
					...
					elseif($arrInvLook[0]["audit_date"] >=$arrInvLook[0]["interbranch_out_past_date"] 
                               ...
                               elseif( ( $arrInvLook[0]["audit_date"] <=$arrInvLook[0]["stock_transfer_status_date"]) 
                                ...
                                // echo "cccc";
                                                    ...
                                                    if(( $arrInvLook[0]["audit_date"] <=$arrInvLook[0]["pullout_past_date"]) ){
                                                        $pullout_past =$arrInvLook[0]["pullout"];
                                                    ...
                                                    // echo "aaa";
                                                    $beg_inventory = $arrInvLook[0]["beg_inventory"]-$arrInvLook[0]["pullout"]-$arrInvLook[0]["damage"]-$arrInvLook[0]["stock_transfer_out"]
                                                    -$arrInvLook[0]["sales_past"]-$arrInvLook[0]["interbranch_out_past"]+$arrInvLook[0]["interbranch_in_past"]-$arrInvLook[0]["stock_transfer_minus"];
                                                    }
            }
...
function labChecker_auditor($product_code,$store_id,$date_start,$date_end){
    ...
    || preg_match("/PL/i", $product_code) || preg_match("/60319/i",$product_code)
                        || preg_match("/DMP/i",$product_code)  || preg_match("/HC/i",$product_code)  || preg_match("/DD/i",$product_code)   || preg_match("/P1/i",$product_code) || preg_match("/AFC/i", $product_code) 
    ...
    if($store_id=='6981YR1OBFVPK2V40HY8'){
        ...
        // ) ";
        $KCC="   AND   '1'= CASE WHEN o.store_id='151' AND os.lens_code NOT IN ('L001','L002','L045','L013', 'L014', 'L015', 'L016', 'L017', 'L018', 
         ...
         AND ( ".$packaging_for."
                            (
                                (os.lens_option='with prescription' )
                                    AND
                                    os.lens_code NOT IN ('L013', 'L014', 'L015', 'L016', 'L017', 'L018', 'L020', 'L021', 'L022', 'L023', 'L024', 'L029', 'L030', 'L031', 'L032', 'L033',
                                    'SO1001', 'L049', 'L050', 'L051', 'L052', 'L053','SO001' ,'SO002','SO003',
                                    'ML0013', 'ML0014', 'ML0015', 'ML0016', 'ML0018', 'ML0020', 'ML0021', 'ML0022', 'ML0023', 'ML0024', 'ML0029', 
