...
require $sDocRoot."/inventory/includes/s_pd_functionv2.php";
...
function GetName($store_id){
	...
	if($storelength=='3' || $storelength=='7'){
		$query="SELECT store_name_proper FROM store_codes_studios where store_code='".$store_id."' ";
					...
					<?php for($i=0;$i<sizeof($GetData);$i++){?>
						...
						?></td>
							<td><?php if($type_filter=='store'){?>
								<a href='/studios/inventory/store/studios-history/?ref_num=<?=  $GetData[$i]["reference_number"]; ?>'  target='_blank'><?= $GetData[$i]["reference_number"]; ?> </a>
								<?php
						}else{ ?>

						<a href='/studios/inventory/admin/studios-history/?ref_num=<?= $GetData[$i]["reference_number"]; ?>' target='_blank'>	<?= $GetData[$i]["reference_number"]; ?> </a>
