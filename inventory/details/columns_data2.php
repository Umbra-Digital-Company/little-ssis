<?php
	...
	$product_code=str_replace("%20"," ",$_GET['product_code']);	
		...
		elseif($storelength>'10'){
			$branch ='lab';
			 $arrData[] =  labChecker_smr($product_code,$_GET['filterStores'],$_GET['dateStart'],$_GET['dateEnd']);
		}else{
	...
	if(empty($arrData)){
        ...
        $arrColumnData['damage_in_c']  = '0';
        $arrColumnData['damage_in_past']  = '0';
        $arrColumnData['damage_in_past_date']  = '0';
        ...
        $arrColumnData['damage_in_c']  = $arrData[0][0]['damage_in_c'];
        $arrColumnData['damage_in_past']  = $arrData[0][0]['damage_in_past'];
        $arrColumnData['damage_in_past_date']  = $arrData[0][0]['damage_in_past_date'];
		...
		if($branch=='warehouse/' ){
				...
				if(strpos($arrColumnData["past_variance"],"-")){
					...
					$beg_inventory=$beg_inventoryx-$arrColumnData["past_variance"];
						...
						$beg_inventory=$beg_inventoryx+$arrColumnData["past_variance_2"];
				}
			}else{

		if(strpos($arrColumnData["past_variance"],"-")){
		
					$beg_inventory=$arrColumnData["past_variance"];
					...
					elseif( ( $arrColumnData["audit_date"] <=$arrColumnData["stock_transfer_status_date"]) 
							...
							||  ( $arrColumnData["audit_date"] <= $arrColumnData["interbranch_in_past_date"])
								...
								if($arrColumnData["audit_date"] <=$arrColumnData["pullout_past_date"]){
									$pullout_past =  $arrColumnData["pullout"];

								}else{
									$pullout_past = "0";
								}

                                if($arrColumnData["audit_date"] <=$arrColumnData["damage_in_past_date"]){
									$damage_in_past =  $arrColumnData["damage_in_past"];
		...
		//running
	   	...
	   	$runningtotal =  $beg_inventory +$arrColumnData["stock_transfer_in_c"]
								...
								$arrColumnData["interbranch_out_c"]-$arrColumnData["damage_c"]-$arrColumnData["pullout_c"]-$arrColumnData['sales']+$arrColumnData['damage_in_c']; 
