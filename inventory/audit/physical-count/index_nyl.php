...
if ( isset($_GET['filterStores']) && $_GET['filterStores'] != '' ) {
	...
	require $sDocRoot."/inventory/includes/grab_poll_51_test.php";
	require $sDocRoot."/inventory/includes/checker_functionsv3.php";
			...
			<div class="row" style="max-width: 100%;">
 				...
 				<div class="col-12 col-lg-8" style="overflow:hidden">
					...
					<div class="d-flex no-gutters align-items-center" id="data-filter">
						...
						<label for="submitPhysicalCount" class="btn btn-primary phSubmit">Submit Physical Count</label>
					</div>
					...
					<div style="padding: 5px;display: flex; justify-content: flex-end;">
						<div class="col-4" style="padding-right: 0px; padding-left: 0px;">
							...
							<span style="margin-left: 8px" float="right" id="total_r">0</span>
						</strong>
						</div>
							...
							for ($i=0;$i<sizeof($arrPoll51_items);$i++) {
										...
										if($branchType=='warehouse'){
											...
											$running_product[$arrPoll51_items[$i]['product_code'] ] ="0";
                                             ...
                                             if($arrPoll51_items[$i]['product_code']==$running_product[$arrPoll51_items[$i]['product_code']][0]["product_code"]){
                $beg_inventory =$running_product[$arrPoll51_items[$i]['product_code']][0]["past_variance_2"];
                
            }
            else{
                if(  ($running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] >=$running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_status_date"] 
																 &&  !empty($running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_status_date"] )  && empty($running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_out_past_date"]  )  )
															
														){
															
													
                                                            $beg_inventory=$running_product[$arrPoll51_items[$i]['product_code']][0]["past_variance_2"] -$running_product[$arrPoll51_items[$i]['product_code']][0]["sales_deduct_physical"];
                                                            

														}
					elseif($running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] >=$running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_out_past_date"] 
														&&  !empty($running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_out_past_date"]) && empty($running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_status_date"] ) ){


															$beg_inventory=$running_product[$arrPoll51_items[$i]['product_code']][0]["past_variance_2"] -$running_product[$arrPoll51_items[$i]['product_code']][0]["sales_deduct_physical"];
														}
                               elseif( ( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <=$running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_status_date"]) 
                                            || ( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <=$running_product[$arrPoll51_items[$i]['product_code']][0]["damage_past_date"]) 
                                        || ($running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"]!=''  &&  empty($running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_status_date"]))
                                        || ( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <= $running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_minus_date"]) 
                                        || ( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <= $running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_out_past_date"])
                                        ||  ( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <= $running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_in_past_date"])
                                        ){
                                // echo "cccc";
                                                    if( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <=$running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_status_date"] || 
                                                    ( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"]!=''  &&  empty($running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_status_date"])) ) {
                                                        
                                                        $stok_transfer_beg=$running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_in_past"];
                                                    }else{
                                                        $stok_transfer_beg="0";
                                                    ...
                                                    if( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <=$running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_in_past_date"] || 
                                                    ( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"]!=''  &&  empty($running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_in_past_date"])) ) {
                                                        ;
                                                        $interbranch_in_past=$running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_in_past"];
                                                    }else{
                                                    ...
                                                    if( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <=$running_product[$arrPoll51_items[$i]['product_code']][0]["damage_past_date"]) {
                                                            $damage_beg =$running_product[$arrPoll51_items[$i]['product_code']][0]["damage"];
                                                    }else{
                                                        $damage_beg ="0";
                                                    }

                                                    if( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <= $running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_out_past_date"]) {
                                                        $past_interbranch =$running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_out_past"];
                                                }else{
                                                    $past_interbranch ="0";
                                                }
                                                
                                                    
                                                    if(( $running_product[$arrPoll51_items[$i]['product_code']][0]["audit_date"] <=$running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_minus_date"]) ){
                                                        $stock_transfer_beg_minus =$running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_minus"];
                                            ...
                                            // echo "bbb";
                                            $beg_inventory= $running_product[$arrPoll51_items[$i]['product_code']][0]["past_variance_2"]+ $stok_transfer_beg
                                            +$interbranch_in_past
                                            - $running_product[$arrPoll51_items[$i]['product_code']][0]["sales_deduct_physical"]
                                            - $damage_beg
                                            ...
                                            // echo "aaa";
                                            $beg_inventory = $running_product[$arrPoll51_items[$i]['product_code']][0]["beg_inventory"]-$running_product[$arrPoll51_items[$i]['product_code']][0]["pullout"]-$running_product[$arrPoll51_items[$i]['product_code']][0]["damage"]-$running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_out"]
                                            -$running_product[$arrPoll51_items[$i]['product_code']][0]["sales_past"]-$running_product[$arrPoll51_items[$i]['product_code']][0]["transit_out"];
                                            }
            }
    
    
                $runningtotal[$arrPoll51_items[$i]['product_code'] ]=  $beg_inventory +$running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_in_c"]
                    +$running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_in_c"]- $running_product[$arrPoll51_items[$i]['product_code']][0]["stock_transfer_out_c"]-
                    $running_product[$arrPoll51_items[$i]['product_code']][0]["interbranch_out_c"]-$running_product[$arrPoll51_items[$i]['product_code']][0]["damage_c"]-$running_product[$arrPoll51_items[$i]['product_code']][0]["pullout_c"] -$running_product[$arrPoll51_items[$i]['product_code']][0]["sales"]; 
										...
										<tr class="row100 body">
											...
											<td><?=  $runningtotal[$arrPoll51_items[$i]['product_code'] ] ?></td>

											<td style="max-width:100px;min-width:100px;width:100px" nowrap class="cell100 small text-center">
												
												<input type="hidden" name="running[]" value="<?=  $runningtotal[$arrPoll51_items[$i]['product_code'] ] ?>">							
												<input type="hidden" name="product_code[]" value="<?= $arrPoll51_items[$i]['product_code'] ?>">

												<?php

													// Check if import isset
													if(isset($arrCSVData[$arrPoll51_items[$i]['product_code']])) {

														$curPhysicalCount = $arrCSVData[$arrPoll51_items[$i]['product_code']];
