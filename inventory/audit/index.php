...
$filter_page = 'stock_movement_auditor';
...
if(isset($_GET['filterStores'])){
    ...
    elseif($_GET['filterStores']=='warehouse_damage'){
		$branchName ='Warehouse Damage';
	}
    elseif($_GET['filterStores']=='warehouse_qa'){
		$branchName ='Warehouse QA';
	}
	else{
	...
	if($_SESSION['user_login']['userlvl'] == '13' || $_SESSION['user_login']['userlvl'] == '1' || $_SESSION['user_login']['userlvl'] == '15'){
		$branchName = "Warehouse";
		$store_id='warehouse';
						...
						<div class="row mt-3 store-form">
							...
							<div class="col-12">
								<select name="filterStores" id="filterStores" class="select2 form-control">
                                    ...
                                    <option value="warehouse_qa" <?php if($branchtype=='warehouse_qa'){ ?> selected <?php } ?>>Warehouse QA</option>
                                    <option value="warehouse_damage" <?php if($branchtype=='warehouse_damage'){ ?> selected <?php } ?>>Warehouse Damage</option>
									</optgroup>
				...
				<div class="col">
					<div class="d-flex align-items-center">
						...
						<section class="ml-0 ml-md-3">
							<p class="h3 font-bold"><?= ucwords(str_replace(['ali','sm','mw','mtc','hq','-'],['ALI','SM','MW','MTC','HQ',' '],strtolower($branchName))) ?></p>
					...
					<table class="table-striped table-inventory">
						<thead>
							...
							// date('Y-m-d',strtotime($dateStartpdh .'+'.$x.' days')) ;
								...
								<?php } ?> -->
                                <th class="cell100 text-uppercase text-center small column3 toggle-column" id="col-damage-in" nowrap>damage (+)</th>
								<!-- <?php for ($x=1;$x<=$dayscount;$x++) { ?>
...
for ($i=0;$i<sizeof($arrPoll51_items);$i++) {
							...
							<tr class="row100 body" filterStores="<?= $_GET['filterStores'] ?>" product_code="<?= $arrPoll51_items[$i]["product_code"] ?>" dateStart="<?= $dateStart ?>" dateEnd="<?= $dateEnd ?>" datecurrentH ="<?= $datecurrentH ?>" dateStartpdh ="<?= $dateStartpdh ?>" dateEndpdh ="<?= $dateEndpdh ?>">
                                    ...
                                    <td nowrap class="cell100 small text-center damage_in_value">
										-
									</td>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#btn_download_all").click(function(){
			...
			$.post("/process/inventory/admin/download_warehouse_v2.php",
		...
		function getDataHeaderAll(){
			$(".table-inventory thead tr").each(function(){
				...
				arrHeader.push({
					...
					header_damage_in : arrThData[8],
					header_damage : arrThData[9],
					header_in_transit_plus : arrThData[10],
					header_in_transit_minus : arrThData[11],
					header_running_inventory : arrThData[12],
					header_physical_count : arrThData[13],
					header_variance : arrThData[14]
				})
			});
		}
		function getDataToDownloadAll(){
			$(".table-inventory tbody tr").each(function(){
				...
				arrData.push({
					...
					get_damage_in : $.trim($(this).find(".damage_in_value").text()),
		...
		let count_r = 0;
		 $(".table-inventory tbody tr").each(function(){
	    	...
	    	$.get("/inventory/details/columns_data2.php",{filterStores:encodeURIComponent(filter_stores),product_code:encodeURIComponent(product_code),dateStart:encodeURIComponent(ds),dateEnd:encodeURIComponent(de), datecurrentH:encodeURIComponent(datecurrentH), dateStartpdh:encodeURIComponent(dateStartpdh), dateEndpdh:encodeURIComponent(dateEndpdh)}, function(result){
                ...
                _this.find('.damage_in_value').text((result.damage_in_c != null) ? result.damage_in_c : 0 );
