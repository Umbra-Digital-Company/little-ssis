	...
	<div id="ssis-main" class="col <?= str_replace(' ','-',$page_url) ?>">
		...
		<div class="ssis-content">
			...
			<?php if ( isset($_GET['id']) && $_GET['id'] != '' ) { ?>
				...
				<form action="/inventory/process/store/receive.php" method="POST" id="receive-form">
							...
							<div class="custom-card">
								<div class="row">
									<div class="col-6">
										...
										<input type="text" name="stock_from_branch" class="form-control" value="<?= $arrReceivableItems[0]['stock_from_id'] == 'warehouse' ? 'Warehouse' : ucwords(str_replace(['ali','mw','sm'],['ALI','MW','SM'],strtolower($arrReceivableItems[0]['stock_from_branch']))) ?>" readonly>
									</div>
									<div class="col-6">
										<h5 class="text-secondary font-bold font-bold">To</h5>
										<input type="hidden" name="stock_to_id" value="<?= $_SESSION['user_login']['store_code'] ?>">
										<input class="form-control" value="<?= ucwords(str_replace(['ali','mw','sm'],['ALI','MW','SM'],strtolower($storeName))) ?>" readonly>
									</div>
								</div>
						...
						<div class="col-12 col-lg-4">
								...
								<div class="mt-3">
									...
									<div class="custom-card">
										<div class="canvas-holder text-center">
											<canvas id="thecanvasNew" width="475" height="148" border="0"></canvas>
							...
							<div class="mt-5">
								<div class="d-flex justify-content-end">
									<?php if ( $arrReceivableItems[0]['status']=='in transit') { ?>
										<button type="submit" class="btn btn-primary" id="confirmFormNew" style="display: none;">receive</button>
									<?php } ?>
									<a href="/inventory/store/receive/"><button type="button" class="btn btn-secondary ml-3">Exit</button></a>
								</div>
							</div>
							...
							<table class="table table-striped mb-0">
								...
								<tbody>
									...
									<?php for ( $i=0; $i<sizeof($arrReceivable); $i++ ) { ?>
										...
										<tr>
											<td nowrap class=""><?= $arrReceivable[$i]['stock_from_id'] == 'warehouse' ? 'Warehouse' : ucwords(str_replace(['ali','mw','sm','mtc','-'],['ALI','MW','SM','MTC',' '],strtolower($arrReceivable[$i]['stock_from_branch']))) ?></td>
											<td nowrap class=""><?= ucwords(str_replace(['ali','mw','sm','mtc','-'],['ALI','MW','SM','MTC',' '],strtolower($storeName))) ?></td>
											...
											<td nowrap class=""><a href="/inventory/store/receive/?id=<?= $arrReceivable[$i]['reference_number'] ?>" class="text-success text-uppercase font-weight-bold"><?= $arrReceivable[$i]['status']=='in transit' ? 'receive' : 'view' ?></a></td>
										</tr>
