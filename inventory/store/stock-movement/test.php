					...
					<table class="table-striped table-inventory">
                        ...
                        <?php 
							...
							for ($i=0;$i<sizeof($arrDailySales);$i++) {
								...
								$arrXSalesDateCount[$arrDailySales[$i]["product_code"]][$i]=$arrDailySales[$i]["payment_date"];


								$arrSalesDay[$arrDailySales[$i]["product_code"]]["total_frame"] = "0";
								...
								$arrSalesDay[$arrDailySales[$i]["product_code"]][$arrDailySales[$i]["payment_date"]]["sales_count"]="0";
...
// exit;
							for ($i=0;$i<sizeof($arrFrames);$i++) { 
								...
								<tr class="row100 body">
                                    ...
                                    <?php 
										...
										$beg_inventoryx =$arrFrames[$i]["beginning_inventory"]-$arrFrames[$i]["pullout"]-$arrFrames[$i]["damage"]-$arrFrames[$i]["stock_transfer_out"]
											-$sales_past-$arrFrames[$i]["interbranch_out"]- $arrFrames[$i]["transit_out_past"];
											// $past_variance= $beg_inventoryx - $arrFrames[$i]["past_variance"];
											// echo "<br>";
                                    ...
                                    if( in_array($arrFrames[$i]['product_code'],$arrXSales	) ){
                                        if($arrFrames[$i]['product_code']==$arrSalesDay[$arrFrames[$i]['product_code']]["product_code"]){
                                            // echo "a";
											$sale_frame =$arrSalesDay[$arrFrames[$i]['product_code']]["total_frame"];
									...
									<?php
								  		for($x=0;$x<=$dayscount;$x++) {
                                            ...
                                            if( in_array($arrFrames[$i]['product_code'],$arrXSales	) && in_array($stock["sales_per_day"][$x],$arrXSalesDate[$arrFrames[$i]['product_code']]) ){
                                                    ...
                                                    if($arrFrames[$i]['product_code']==$arrSalesDay[$arrFrames[$i]['product_code']][$stock["sales_per_day"][$x]]["product_code"]
                                                        && $stock["sales_per_day"][$x] ==$arrSalesDay[$arrFrames[$i]['product_code']][$stock["sales_per_day"][$x]]["status_date"]
                                                    ){
                                                        // $tot_sale[$arrFrames[$i]['product_code']] ="0";
                                                        // $tot_sale[$arrFrames[$i]['product_code']] +=$arrSalesDay[$arrFrames[$i]['product_code']][$stock["sales_per_day"][$x]]["sales_count"];
                                                        $sale_frame_day=$arrSalesDay[$arrFrames[$i]['product_code']][$stock["sales_per_day"][$x]]["sales_count"];
                                                    }else{
                                                        // $tot_sale[$arrFrames[$i]['product_code']] ="0";
                                                        $sale_frame_day="0";
                                                    } 
									...
									<td nowrap class="cell100 small text-center"><?php 
										...
										echo $arrFrames[$i]["stock_transfer_in_c"];
									...
									<td nowrap class="cell100 small text-center"><?php
										...
										?>
										<td nowrap class="cell100 small text-center d-none" data-col="inter-in">
											<?php
											$plusDay[$x] = '"+'.$x.' days"';
											//// perday stock_indate('Y-m')
									...
									<?php } ?>
									<td nowrap class="cell100 small text-center">
										<?php echo $arrFrames[$i]["interbranch_out_c"]; ?>
									</td>
									<?php for ($ost=0;$ost<=$dayscount;$ost++) { ?>
										<td nowrap class="cell100 small text-center d-none" data-col="inter-out">
											<?php	 
						...
						if(in_array($arrFrames[$i]['product_code'],$arrframeDayX)){
									...
									<td nowrap class="cell100 small text-center"><?php
											...
											$arrFrames[$i]["interbranch_out_c"]-$arrFrames[$i]["damage_c"]-$arrFrames[$i]["pullout_c"]-$tot_sale[$arrFrames[$i]['product_code']]- $arrFrames[$i]["transit_out"]; 
											echo $runningtotal;
