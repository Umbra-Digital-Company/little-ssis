		...
		<div class="ssis-content">
			...
			<div class="dashboard-filter">
				...
				<form method="GET" class="filter-body">
					...
					<!-- transaction process -->
					<p class="text-uppercase font-bold text-primary">transaction process</p>
					<div class="row mt-3 transaction-process">
						<div class="col-12 col-md-6 mb-2">
							<div class="d-flex align-items-center">
								<input class="sr-only checkbox" name="filterProcess" type="radio" id="optionProcessIn" value="in" <?= (isset($_GET['filterProcess']) && $_GET['filterProcess']=='in') ? 'checked="checked"' : '' ?>>
								<label for="optionProcessIn" class="custom_checkbox"></label>
								<label for="optionProcessIn" class="m-0 ml-2">Going In</label>
							</div>
						</div>
						<div class="col-12 col-md-6 mb-2">
							<div class="d-flex align-items-center">
								<input class="sr-only checkbox" name="filterProcess" type="radio" id="optionProcessOut" value="out" <?= (isset($_GET['filterProcess']) && $_GET['filterProcess']=='out') ? 'checked="checked"' : '' ?>>
								<label for="optionProcessOut" class="custom_checkbox"></label>
								<label for="optionProcessOut" class="m-0 ml-2">Going Out</label>
							</div>
						</div>
						<div class="col-12 col-md-6 mb-2">
							<div class="d-flex align-items-center">
								<input class="sr-only checkbox" name="filterProcess" type="radio" id="optionAllProcess" value="all" <?= ( (isset($_GET['filterProcess']) && $_GET['filterProcess']=='all')) || !isset($_GET['filterProcess']) ? 'checked="checked"' : '' ?>>
								<label for="optionAllProcess" class="custom_checkbox"></label>
								<label for="optionAllProcess" class="m-0 ml-2">All</label>
							</div>
						</div>
					</div>
					...
					<div class="row mt-2">
						<div class="col-12 col-md-6">
							<div class="d-flex align-items-center">
								<p class="mr-2">From:</p>
								<select name="filterSender" id="filterSender" class="form-control select2" <?= ( (isset($_GET['filterProcess']) && $_GET['filterProcess']!='in') || !isset($_GET['filterProcess'])) ? 'disabled="disabled"' : '' ?>>
									<option value="">All Branch</option>
									<optgroup label="STORE NAME">
										<?php for ($i=0;$i<sizeof($arrStore);$i++) { ?>
											<option value="<?= $arrStore[$i]['store_id'] ?>" <?= (isset($_GET['filterSender']) && $_GET['filterSender']==$arrStore[$i]['store_id']) ? 'selected="selected"' : ''; ?>><?= ucwords(str_replace(['ali','mw','sm','-'], ['ALI','MW','SM',' '], strtolower($arrStore[$i]['store_name']))) ?></option>
										<?php } ?>
									</optgroup>
						...
						<div class="col-12 col-md-6">
							<div class="d-flex align-items-center">
								<p class="mr-2">To:</p>
								<select name="filterReceiver" id="filterReceiver" class="form-control select2" <?= ( (isset($_GET['filterProcess']) && $_GET['filterProcess']!='out') || !isset($_GET['filterProcess'])) ? 'disabled="disabled"' : '' ?>>
									<option value="">All Branch</option>
									<optgroup label="STORE NAME">
										<?php for ($i=0;$i<sizeof($arrStore);$i++) { ?>
											<option value="<?= $arrStore[$i]['store_id'] ?>" <?= (isset($_GET['filterReceiver']) && $_GET['filterReceiver']==$arrStore[$i]['store_id']) ? 'selected="selected"' : ''; ?>><?= ucwords(str_replace(['ali','mw','sm','-'], ['ALI','MW','SM',' '], strtolower($arrStore[$i]['store_name']))) ?></option>
										<?php } ?>
									</optgroup>
									<optgroup label="LAB NAME">
										<?php for ($i=0;$i<sizeof($arrLab);$i++) { ?>
					...
					<!-- transaction type -->
					<p class="text-uppercase font-bold text-primary mt-4">transaction type</p>
					...
					<div class="text-center mt-5 pb-5">
						...
						<?php if(isset($_GET['date'])) { ?>
							...
							<a href="/inventory/warehouse/history/" class="ml-0 mt-3 mt-sm-0 mt-sm-3 d-block d-sm-inline ml-sm-3">
			...
			<div class="d-flex no-gutters align-items-center" id="data-filter">
				...
				<?php if(isset($_GET['ref_num'])){
					...
					<input type="text"name="ref_num" class="search form-control col" placeholder="Search reference number" value="<?= $ref_num!='' ? $ref_num : '' ?>">
			...
			<div id="inventory-receive" class="mt-4">
								...
								<tbody>
									...
									<?php for ( $i=0; $i<sizeof($arrReceivable); $i++ ) { ?>
										...
										<tr>
											...
											<td nowrap class=""><?= ucwords(str_replace(['mw','ali','mtc','sm','hq','qa','-'],['MW','ALI','MTC','SM','HQ','QA',' '],strtolower($arrReceivable[$i]['stock_from_branch']))) ?></td>
											<td nowrap class=""><?= ucwords(str_replace(['mw','ali','mtc','sm','hq','qa','-'],['MW','ALI','MTC','SM','HQ','QA',' '],strtolower($arrReceivable[$i]['stock_to_name']))) ?></td>
				...
				<?php } else { ?>
					...
					<div class="mt-5 text-center">
						<a href="/inventory/warehouse/history/" class="text-uppercase text-danger">clear search</a>
					</div>
...
<script>
	...
	$(document).ready(function() {
		...
		// Filter Process Type
		$('input[name="filterProcess"]').on('click', function() {
			var id = $(this).attr('id');
			
			switch ( id ) {
				case 'optionProcessIn' :
					$('#filterReceiver').prop('disabled', true).val('');
					$('#filterSender').prop('disabled', false);
					break;

				case 'optionProcessOut' :
					$('#filterSender').prop('disabled', true).val('');
					$('#filterReceiver').prop('disabled', false);
					break;
