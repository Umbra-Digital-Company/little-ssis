...
require $sDocRoot."/inventory/includes/warehouse_function.php";
            ...
            // echo "</pre>";
								...
								<tr class="row100 body">
									...
									<td nowrap class="cell100 small text-center">
												<?php 
													// $beginning_inventory =GetTotalBegInventory(arrPoll51_items[$i]['product_code'],$inventory_user,$i_date_start,$i_date_end);
													// echo $beginning_inventory;
													// echo arrPoll51_items[$i]["beginning_inventory"];
													$beg_inventory =$FrameData[$arrPoll51_items[$i]["product_code"]][0]["beg_inventory"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["pullout"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["damage"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["stock_transfer_out"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["sales"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["transit_out"];
													echo $beg_inventory;
                                                    // echo arrPoll51_items[$i]["stock_transfer_out"];
                                                    	
												?>
									</td>

									<td nowrap class="cell100 small text-center"><?= $FrameData[$arrPoll51_items[$i]["product_code"]][0]["requested"]?></td>
									
									<td nowrap class="cell100 small text-center">
										<?php echo  $FrameData[$arrPoll51_items[$i]["product_code"]][0]["stock_transfer_in_c"];?>
									</td>
									<?php  for ($x=0;$x<=$dayscount;$x++) { ?>
										<td nowrap class="cell100 small text-center d-none" data-col="stock-transfer-in">
														...
														// if(in_array($arrPoll51_items[$i]['product_code'],$arrframeDayX)){
														// 	if($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_in_date["stock_transfer_in"]]["stock_in"]["status_date"]==$stock_in_date["stock_transfer_in"] 
														// 	&&  ($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_in_date["stock_transfer_in"]]["stock_in"]["type"]=='stock_transfer'
														// 		|| $arrframeday[$arrPoll51_items[$i]['product_code']][$stock_in_date["stock_transfer_in"]]["stock_in"]["type"]=='replenish')
														// 	&& 	($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_in_date["stock_transfer_in"]]["stock_in"]["store_id"]==$_SESSION['store_code'] 
														// 	&& $arrframeday[$arrPoll51_items[$i]['product_code']][$stock_in_date["stock_transfer_in"]]["stock_in"]["status"]=='received'
																	
														// 		) ){
														// 		echo	$stock_in_per_day=$arrframeday[$arrPoll51_items[$i]['product_code']][$stock_in_date["stock_transfer_in"]]["stock_in"]["total"];
														// 	}else{
									...
									<?php for ($ost=0;$ost<=$dayscount;$ost++) { ?>
										<td nowrap class="cell100 small text-center d-none" data-col="stock-transfer-out">
											...
											// if(in_array($arrPoll51_items[$i]['product_code'],$arrframeDayX)){
											// 		if($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["stock_transfer_out"]]["stock_out"]["status_date"]==$stock_out_date["stock_transfer_out"] 
											// 		&&  ($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["stock_transfer_out"]]["stock_out"]["type"]=='stock_transfer' 
											// 				)
											// 		&& $arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["stock_transfer_out"]]["stock_out"]["stock_from"]==$_SESSION['store_code'] 
											// 		&& $arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["stock_transfer_out"]]["stock_out"]["status"]=='received')
											// 		{
											// 			echo 	$stock_out_per_day=$arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["stock_transfer_out"]]["stock_out"]["total"];
											// 		}else{
									...
									<td nowrap class="cell100 small text-center"><?= $FrameData[$arrPoll51_items[$i]["product_code"]][0]['pullout_c'] ?></td>
											...
											<?php for ($ost=0;$ost<=$dayscount;$ost++) { ?>
												<td nowrap class="cell100 small text-center d-none" data-col="pullout">
														...
														// if(in_array($arrPoll51_items[$i]['product_code'],$arrframeDayX)){
														// 	if($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["pullout"]]["pullout"]["status_date"]==$stock_out_date["pullout"] 
														// 	&&  ($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["pullout"]]["pullout"]["type"]=='pullout' 
														// 			)
														// 	&& $arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["pullout"]]["pullout"]["stock_from"]==$_SESSION['store_code']
														// 	&& $arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["pullout"]]["pullout"]["status"]=='received' ){
														// 		echo 	$pulloutday=$arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["pullout"]]["pullout"]["total"];
														// 	}else{
														// 		echo $pulloutday="0";
														// 	}
														// }else{
											...
											<?php } ?>
									<td nowrap class="cell100 small text-center"><?= $FrameData[$arrPoll51_items[$i]["product_code"]][0]['damage_i'] ?></td>
									<?php for ($ost=0;$ost<=$dayscount;$ost++) { ?>
										<td nowrap class="cell100 small text-center d-none" data-col="damage">
											...
											// if(in_array($arrPoll51_items[$i]['product_code'],$arrframeDayX)){
											// 		if($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["damage"]]["damage"]["status_date"]==$stock_out_date["damage"] 
											// 		&&  ($arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["damage"]]["damage"]["type"]=='damage' 
											// 				)
											// 		&& $arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["damage"]]["damage"]["store_id"]=='warehouse_damage'
											// 		&& $arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["damage"]]["damage"]["status"]=='received'){
											// 			echo 	$damage=$arrframeday[$arrPoll51_items[$i]['product_code']][$stock_out_date["damage"]]["damage"]["total"];
											// 		}else{
											// 			echo $damage="0";
											// 		}
											// 	}else{
									...
									<td nowrap class="cell100 small text-center"><?php 
										...
										// $total_in_transit= GetTotalOnTransit($arrPoll51_items[$i]['product_code'],$inventory_user,$i_date_start,$i_date_end);
									...
									<td nowrap class="cell100 small text-center"><?php 
										...
										// $total_in_transit2= GetTotalOnTransit2($arrPoll51_items[$i]['product_code'],$inventory_user,$i_date_start,$i_date_end);
...
$runningtotal=  $beg_inventory +$FrameData[$arrPoll51_items[$i]["product_code"]][0]["stock_transfer_in_c"]
    ...
    $FrameData[$arrPoll51_items[$i]["product_code"]][0]["interbranch_out_c"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["damage_c"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["pullout_c"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["sales"]-$FrameData[$arrPoll51_items[$i]["product_code"]][0]["transit_out"]; 
										echo $runningtotal;
									...
									<td nowrap class="cell100 small text-center"><?php 
												// echo $dateEndpdh;
												// echo "<br>";
                                                // echo date('Y-m-d',$arrActual[$arrPoll51_items[$i]['product_code']]["date_end"];
                                                if(in_array($arrPoll51_items[$i]['product_code'],$arrActualX )){
												if($arrActual[$arrPoll51_items[$i]['product_code']]["date_end"]==$dateEndpdh 
												&& $arrPoll51_items[$i]['product_code']==$arrActual[$arrPoll51_items[$i]['product_code']]["product_code"]){

															$actual=$arrActual[$arrPoll51_items[$i]['product_code']]["count"];
												}else{
														$actual="0";
                                                }
                                            }else{
