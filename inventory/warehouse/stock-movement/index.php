...
$filter_page = 'stock_movement_warehouse';
...
<div class="row no-gutters align-items-strech flex-nowrap">
	...
	<div id="ssis-main" class="col <?= str_replace(' ','-',$page_url) ?>">
		...
		elseif($_SESSION['store_code']=='warehouse_damage'){
			$branch ='warehouse_damage';
			//$FrameData[$arrPoll51_items[$i]["product_code"]]= WarehouseChecker_smr($arrPoll51_items[$i]["product_code"],$dateStart,$dateEnd);
		}
		elseif($_SESSION['store_code']=='warehouse_qa'){
			$branch ='warehouse_qa';
			//$FrameData[$arrPoll51_items[$i]["product_code"]]= WarehouseChecker_smr($arrPoll51_items[$i]["product_code"],$dateStart,$dateEnd);
		}
		else{
...
for ($i=0;$i<sizeof($arrPoll51_items);$i++) {
							...
							<tr class="row100 body" filterStores="<?= $branch?>" product_code="<?= $arrPoll51_items[$i]["product_code"] ?>" dateStart="<?= $dateStart ?>" dateEnd="<?= $dateEnd ?>" datecurrentH ="<?= $datecurrentH ?>" dateStartpdh ="<?= $dateStartpdh ?>" dateEndpdh ="<?= $dateEndpdh ?>">
