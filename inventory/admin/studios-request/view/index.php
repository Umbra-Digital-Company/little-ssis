...
$page = 'admin';
$page_url = 'Edit current request Studios';

$filter_page = 'current_request_admin_studios';
...
<div class="row no-gutters align-items-strech flex-nowrap">
	...
	<div id="ssis-main" class="col <?= str_replace(' ','-',$page_url) ?>">
		...
		<form action="/inventory/process/admin/edit_request.php" method="POST" id="pullout-form" autocomplete="off">
									...
									<input type="hidden" name="reference" value="<?= $arrReceivable[0]['reference_number'] ?>">
										<?php for ($i=0; $i < sizeOf($arrReceivable); $i++) {
											...
											echo 	'<td style="vertical-align:middle!important;min-width:300px;">'.$arrReceivable[$i]['item_remarks'].'</td>';
												echo '<input type="hidden" name="off[]" value="'.$arrReceivable[$i]['product_code'].'">';
												
											echo 	'<td style="vertical-align:middle!important;min-width:100px;max-width:100px;" class="text-center"><input type="text" class="form-control"  name="count['.$arrReceivable[$i]['product_code'].']" value="'.$arrReceivable[$i]['count'].'"></td>';
