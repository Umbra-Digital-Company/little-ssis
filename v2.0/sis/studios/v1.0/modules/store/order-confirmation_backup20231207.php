...
if(!isset($_SESSION['customer_id'])) { ?>
    ...
    <div class="wrapper">
        ...
        <div class="text-center mt-4">
            <a href="/v2.0/sis/studios/v1.0/?page=store-home"><button class="btn btn-primary"><?= $arrTranslate['Go to step 1'] ?></button></a>
        </div>    
    </div>
        ...
        <?php } ?>
                    ...
                    <div class="card mt-3 card-others" style="background-color: #E8E8E4 !important">
                        ...
                        ?>
                            <div class="card-body pt-3 pb-1 pl-1">
                                ...
                                <div class="d-flex justify-content-between">
                                    <div class="col mr-4">
                                        <p style="font-size: 12px;"><?= $arrTranslate['Are you a Sunnies Club Member?'] ?></p>
                                    </div>
                                    <div class="d-flex justify-content-between pb-3">
                                        <div class="d-flex align-items-center radio">
                                            <input type="radio" name="club_member" id="club_member_yes" class="sr-only club_member checkbox" value="yes" <?= $arrCart[0]['club_member'] == 'y' ? 'checked' : '' ?>>
                                            <label for="club_member_yes" class="custom_checkbox"></label>
                                            <label for="club_member_yes"><?= $arrTranslate['Yes'] ?></label>
                                        </div>
                                        <div class="d-flex align-items-center radio ml-3">
                                            <input type="radio" name="club_member" id="club_member_no" class="sr-only club_member checkbox"  value="no" <?= $arrCart[0]['club_member'] == 'n' ? 'checked' : '' ?>>
                                            <label for="club_member_no" class="custom_checkbox doctor"></label>
                                            <label for="club_member_no"><?= $arrTranslate['No'] ?></label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <?php if($_SESSION['store_type'] == 'ns' && !strstr($arrCart[0]['email_address'],'guest')){ ?>
                    ...
                    <?php if(isset($arrCart[0]['promo_code']) && $arrCart[0]['promo_code']!=''){ ?>
                        <div class="d-flex justify-content-between ml-4 mr-4">
                            ...
                            <p class="text-uppercase text-primary font-bold mb-3 mt-4">Subtotal</p>
                            <p class="text-uppercase text-primary font-bold mb-3 mt-4"><?= number_format($total_price, 2) ?></p>

                        </div>
                        <?php $voucher_amount = ($arrCart[0]['promo_code_amount'] > 0) ? $arrCart[0]['promo_code_amount'] : 0; ?>
                        <div class="d-flex justify-content-between ml-4 mr-4">
                            <p class="text-uppercase text-primary font-bold mb-3 mt-4"></p>
                            <p class="text-uppercase text-primary font-bold mb-3 mt-4">Voucher Amount</p>
                            <p class="text-uppercase text-primary font-bold mb-3 mt-4"><?= number_format($voucher_amount, 2) ?></p>
            ...
            <div class="row d-flex justify-content-center mt-5" >
                ...
                <div class="col-12">
                    ...
                    <a href="/v2.0/sis/studios/func/process/order_payment.php?path_loc=v1.0" id="send-order"><input type="button" class="btn btn-black" value="<?= $textSend ?>"></a>
                </div>
            </div>            
        </form>
    </section>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $(this).on('click', '.add_count_increment_pbag', function(){
            ...
            if(count_num_val >= total_count){
                if(confirm('Your about to exceed the total count of frames.')){
                    ...
                    $.post("/v2.0/sis/studios/func/process/add_to_bag_merch.php",{studios_product_code: productSelected, paper_bag:true}, function(result){
                        //console.log(result);
        ...
        $(this).on('click','.club_member',function(){
            $('#loading').modal('show');
            $.post("/v2.0/sis/studios/func/process/club_member.php",{club_member:$(this).val()}, function(result){
                setTimeout(()=>{
                    $('#loading').modal('hide');
                },200);
            });
        ...
        $(this).on('click','.set_receipt_holder',function(){
            ...
            if($(this).val() == 'yes'){
                if($(this).attr('orders-specs-id') == ''){
                    $('#loading').modal('show');
                    $.post("/v2.0/sis/studios/func/process/add_to_bag_merch.php",{studios_product_code: $(this).attr('product-code'), paper_bag:true}, function(result){
                ...
                if($(this).attr('orders-specs-id') != ''){
                    $('#loading').modal('show');
                     $.post("/v2.0/sis/studios/func/process/remove_item.php",{orders_specs_id: $(this).attr('orders-specs-id')}, function(){
