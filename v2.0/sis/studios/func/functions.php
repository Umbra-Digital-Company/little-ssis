...
function get_url($url) {
	return '/v2.0/sis/studios/assets/' . $url;
}
...
function grabLanguageTags() {
	...
	// Check lanauge setting
	if(isset($_SESSION['language_setting'])) {
	    $queryCC = $_SESSION['language_setting'];
	}
	else {
	...
	// Sort into desired array structure
	for ($i=0; $i < sizeOf($arrTagsInit); $i++) { 
		
		// Current word
		$curWord = $arrTagsInit[$i]['text'];
		$curTranslation = $arrTagsInit[$i]['translation'];

		$arrTags[$curWord] = $curTranslation;
