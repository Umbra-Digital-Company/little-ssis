...
if(!isset($_SESSION['user_login']['username'])) {
	...
	if(isset($_SESSION['store_code']) && isset($_SESSION['order_no'])){
	    ...
	    if($_SESSION['store_type'] == 'ns'){
	    	$query = 	'UPDATE orders_sunnies_studios
						SET status ="paid"
						WHERE order_id = "'.$order_no.'"
							AND product_upgrade = "LE001";';
		    $stmt = mysqli_stmt_init($conn);
		    if (mysqli_stmt_prepare($stmt, $query)) {
		...
		if(!$error){
			...
			echo "<script>  window.location = '/v2.0/sis/studios/".$_GET['path_loc']."/?page=order-dispatched&order_id=".$order_no."&name=".$name."';</script>";
		}else{
