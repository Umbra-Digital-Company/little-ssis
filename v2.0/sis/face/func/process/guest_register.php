...
function checkProfileID($profile_id){
	...
	return $profileIDF;
			...
			echo "	<script>	window.location='/v2.0/sis/face/".$_GET['path_loc']."/?page=select-store'</script>";

			// $path_loc = isset($_GET['path_loc']) ? htmlspecialchars($_GET['path_loc']) : '';
			// $bpage = isset($_GET['bpage']) ? htmlspecialchars($_GET['bpage']) : '';

			// echo "<script>
			// 		window.location = '/v2.0/sis/face/{$path_loc}/?page=order-confirmation&bpage={$bpage}';
			// 	</script>";
