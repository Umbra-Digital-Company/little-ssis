$(document).ready(function () {
	...
	*/
	...
	if ( $('#login_form').length ) {
		...
		$(".form-control").on('keyup', function(e) {
			if ( e.keyCode == 13 ) ajaxSubmit('/v2.0/sis/face/func/loginConfirm.php', 'login_form');
		});

		$("#btnsubmit").on('click', function() {
			ajaxSubmit('/v2.0/sis/face/func/loginConfirm.php', 'login_form');
		});
	...
	$('#switch_account').on('click', function (e) {
		e.preventDefault();
		$('.ssis-overlay').load("/v2.0/sis/face/func/accounts.php", function (d) {
			overlayContent(d);
			...
			// submit login
			$("#btnsubmit").on('click', function () {
				ajaxSubmit('/v2.0/sis/face/func/process/add_user2.php', 'add_account_login');
			});

			$(".form-control").on('keyup', function (e) {
				if (e.keyCode == 13) ajaxSubmit('/v2.0/sis/face/func/process/add_user2.php', 'add_account_login');;
			});
	...
	$('#add_employee').on('click', function() {
		$('.ssis-overlay').load("/v2.0/sis/face/func/account/create-account.php", function (d) {
			overlayContent(d);
		});
	});
	...
	const loadOverviewConfirmation = form => {
		...
		if (isChecked > 0) {
			$.post("/v2.0/sis/face/func/process/overview_confirmation.php", $(form).serialize(), function (d) {
				overlayContent(d);
			});
		} else {
	...
	const useAccountLogin = () => {
		$.post("/v2.0/sis/face/func/store/store-sign-master.php", $("form#use_account").serialize(), function (d) {
			$('#msg').html("");
	...
	if ( $('.customer-profile-examine').length ) {
		...
		$('#send_to_cashier_overview').on('click', function() {
			...
			sendToCashier.on('click', function() {
				...
				$('.ssis-loading').fadeIn();
				$.ajax({
					url: '/v2.0/sis/face/func/process/send_cashier.php',
					method: 'POST',
