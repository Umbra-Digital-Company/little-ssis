                        ...
                        <div class="d-flex d-flex-m justify-content-between">
                            <div class="product-top" style="text-align:center;">
                                ...
                                <?php
                                    ...
                                    if($curImageURL == '/v2.0/sis/face/assets/images/defaults/no_specs_frame_available_b.png') {
                            ...
                            <div class="product-top" style="text-align:center">
                                ...
                                <div class="d-flex justify-content-center mt-2">
                                    <p style="font-size: 18px;"><strong><?= (isset($_SESSION['store_type']) && trim($_SESSION['store_type']) == 'vs') ? 'VND ' : '₱'?></strong><?= $_GET['price'] ?></p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="frame-list" style="height:62vh; overflow: auto;">     
                    ...
                    ?>
                        ...
                        <?php for($i = 0; $i < $showDataCount; $i++) { ?>                       
                                ...
                                ?> 
                                    <div class="frame-style col-6 mb-3 hide-lazy" data-style="<?= $arrMerchSorted[$i]['item_description'] ?>">
                                        <div class="frame-style__slider">
                                            ...
                                            <div style="background: #e8e8e4; padding: 15px; border-radius: 0 0 10px 10px;">
                                                <section class="product-details row flex-nowrap no-gutters align-items-start justify-content-between">
                                                    <h4><?= $arrMerchSorted[$i]['item_description'] ?>&nbsp;<span class="blk"><?= trimColor($curColors[$a]['color']); ?></span></h4>
                                                </section>
                                                <div class="row d-flex justify-content-center mt-3">
                                                    <form class="col-12 form-quick-add-to-bag" id="form-quick-add-to-bag<?= $i ?>" method="POST">
                                                        <input type="hidden" name="studios_product_code" id="input-sku-<?= trim($arrMerchSorted[$i]['item_description']) ?>" value="<?= trim($curColors[$a]['product_code']) ?>">
                                                        <input type="hidden" class="form-control count_num" name="count_num_value" value="1" readonly>
                                                        <button type="submit" class="btn btn-primary" id="btn-add-<?= strtolower($arrMerchSorted[$i]['item_description']) ?>">add to bag</button>
                                                    </form>
                                                </div>                                
                                            </div>
                                        </div>
...
<?php
    ...
    if(isset($countData) && $countData >= 10){
        for($i = 10; $i < count($arrMerchSorted); $i++){
            $arrProductsSortedToShow[] = $arrMerchSorted[$i];
        }
    }
?>
<script>
    ...
    $(document).ready(function(){
        ...
        $("#cart").click(function(){
            ...
            for(let i =0 ; i<arrCart.length; i++){
                ...
                else if(arrCart[i].product_code_order.toLowerCase().indexOf('P1008') > -1){}
        ...
        $(document).on('submit', "#form-add-to-bag", function(e){
            e.preventDefault();
            $.ajax({
                url: "/v2.0/sis/face/func/process/add_to_bag_merch.php",
                type: "POST",
                data: $(this).serialize(),
                dataType: 'html',
       ...
       $(this).on('click', '.remove_item', function(){
		...
		$('#search_frame').on('keyup', function () {
            ...
            setTimeout(()=>{
                 window.location = '?page=select-merch'+filter+'&shapes='+arrShapes+'&collections='+arrCollections+'&search='+$(this).val()+'&sub-product='+'<?= (isset($_GET['sub-product'])) ? $_GET['sub-product'] : '' ?>'+
