...
<?php if(isset($_GET['guest'])){ ?>
		...
		<div class="wrapper">
			<form action="/v2.0/sis/studios/func/process/guest_register.php?path_loc=v1.0" method="post">
...
<?php }else{ ?>
	...
	<div class="account-content active mt-4 mb-4" id="create-content">
		<form name="create_account" id="create_account" action="/v2.0/sis/studios/func/process/store-register-v2.php" method="POST" autocomplete="off">
						...
						<div class="col-4 form-group">
							<select name="b_day" id="form-birthDay" class="form-control " required>
								...
								<?php
									for($i = 1; $i <=31; $i++){
										$i = ( $i < 10) ? '0'.$i : $i;
										$selected = ($i == $arrBirthDate[2]) ? 'selected' : '';
								?>
									<option value="<?= $i ?>" <?=$selected ?>><?= $i?></option>
								<?php } ?>
							</select>
							<label class="placeholder" for="form-birthDay">Birth Day</label>
						</div>
						<div class="col-4 form-group pl-0 pr-0">
							<select name="b_month" id="form-birthMonth" class="form-control" required>
								...
								<?php
									...
									foreach($arrMonth as $key => $value){
										$selected = ($key == $arrBirthDate[1]) ? 'selected' : '';
								?>
								<option value="<?= $key ?>" <?=$selected ?>><?= $value ?></option>
									<?php } ?>
							</select>
							<label class="placeholder" for="form-birthMonth">Birth Month</label>
						</div>
						<div class="col-4 form-group">
							<select name="b_year" id="form-birthYear" class="form-control " required>
								...
								<?php
									$year = date('Y');
									for($i = $year; $i >=1920; $i--){
										$selected = ($i == $arrBirthDate[0]) ? 'selected' : '';
								?>
									<option value="<?= $i ?>" <?=$selected ?>><?= $i?></option>
								<?php } ?>
							</select>
							<label class="placeholder" for="form-birthYear">Birth Year</label>
						</div>
					</div>
				...
				<div class="col-12 col-md-6">
					<div class="form-group gender">
						<div class="d-flex no-gutters">
							...
							<div class="col">
								<input class="sr-only" type="radio" id="n-a" name="gender" value="N/A" <?php if( (isset($_SESSION['customer_id']) || isset($_SESSION['temp_data']) ) && ($arrCustomer[0]['gender']=='N/A' || ( isset($_SESSION['temp_data']) && $_SESSION['gender']=='N/A') ) ){ ?> checked='checked' <?php  } ?> required>
								<label class="form-control col" for="n-a" style="border-top-left-radius: 5px; border-bottom-left-radius: 5px;font-size:10px;">Prefer not to say</label>
							</div>
						</div>
					</div>
				</div>
