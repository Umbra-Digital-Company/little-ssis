...
<?php }else{ ?>
	...
	<div class="account-content active mt-4 mb-4" id="create-content">
		<form name="create_account" id="create_account" action="/v2.0/sis/face/func/process/store-register-v3.php" method="POST" autocomplete="off">
			...
			<div class="form-row no-gutters mt-3">
				...
				<div class="col-6">
					<div class="form-group">
						<input type="date" name="bdate" class="form-control" id="bdate" value="<?= get_customer_data('birthday') ?>" required>
							...
							<div id='age_range' class="form-control d-flex justify-content-between">
								<?php
									...
									foreach ($arrGender as $key => $value) {
										$selected = ( (isset($_SESSION['customer_id']) || isset($_SESSION['temp_data']) ) && (strtolower($arrCustomer[0]['gender'])== strtolower($key) || ( isset($_SESSION['temp_data']) && strtolower($_SESSION['gender'])== strtolower($key) ) ) ) ? "checked='checked'" : '';
								?>
										<div class="d-flex align-items-center radio">
                                            <input type="radio" name="gender" id="<?=$key ?>" class="sr-only checkbox"  value="<?=$key ?>" <?= $selected ?> >
...
<?php } ?>
<script src="/v2.0/sis/face/assets/js/select2.min.js"></script>
<script>
