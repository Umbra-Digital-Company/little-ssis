...
$grabParams = array("courier_no", "orders_specs_id","order_id");

$query = 	'SELECT  os.courier_no,os.orders_specs_id,os.order_id FROM orders_specs os
					LEFT JOIN 
					...
					and lower(payo.status) not IN ("delivered", "closed","early remitted","canceled","cancelled")
...
for ($i=0; $i < sizeOf($arrIDs); $i++) { 
	...
	$api_key = 'ea93bfb4b21c20137d1bb10f33e95d01';
	$invoice = $arrIDs[$i]['courier_no'];
	$client_id = 'sunnies-specs@payo.asia';
  ...
  $signStr .= $api_key;	
    ...
    "contact" => array(
      ...
      "invoice_no" => $arrIDs[$i]['courier_no'],
      "client_id" => 'sunnies-specs@payo.asia',
			"token"		=> $token
	...
	$url = "http://cod.payo.asia/order/get?invoice_no=".$invoice."&client_id=".$client_id."&token=".$token."";
