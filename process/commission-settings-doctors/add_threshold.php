...
require $sDocRoot."/includes/connect.php";
	function checkDateStoresExits($store_code){
		...
		$arrStores = array();
			$query = "SELECT 
			                ...
			                sl.store_name
			            FROM 
			                commission_threshold_doctors cs
		...
		if(count($arrStores) > 0){
			echo json_encode("commission threshold for ".$arrStores[0]['store_name']." already saved!");
	...
	$arrSearchStore = [];
	$arrSearchStoreGroup = [];
	foreach ($_POST['store_code_threshold'] as $key => $value) {
	...
	foreach ($arrStoresDataSearch as $key => $value) {
		...
		$query = 	'INSERT IGNORE INTO
					...
					VALUES (
						...
						"'.mysqli_real_escape_string($conn,$_POST['threshold']).'",
