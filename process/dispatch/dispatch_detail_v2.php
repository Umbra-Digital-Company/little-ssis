		...
		if($_SESSION['store_code'] != '150' &&  $_SESSION['store_code'] != '142'  &&  $_SESSION['store_code'] != '155' && strtotime($_GET['payment_date']) >= strtotime(date('2021-05-10')) ){
			...
			$arrPaperBag = array();
			 ...
			 $query='SELECT
			        ...
			        FROM
			            poll_51 p
			            WHERE p.item_code IN ("P1007","P1001","P1011","SSWP001","SPW002", "SWB001","SWS001") ORDER BY p.item_name ASC;';
	...
	<?php } ?>
		...
		<div class="form-group dispatch-staff-container">
			<select class="form-control text-left" name="dispatch_doctor" id="dispatch_doctor" required>
				...
				<?= (isset($_GET['product_code']) && isset($_GET['product_upgrade']) && ($_GET['product_code'] == 'M100' || $_GET['product_upgrade'] == 'fashion_lens')) ? '<option value="none">None</option>' : '' ?>
