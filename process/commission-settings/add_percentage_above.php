...
require $sDocRoot."/includes/connect.php";
	...
	function checkDateStoresExits($store_code){
		...
		$arrStores = array();
			$query = "SELECT 
			                ...
			                sl.store_name
			            FROM 
			                commission_percentage_above cs
		...
		if(count($arrStores) > 0){
			echo json_encode("Commission above threshold percentage for ".$arrStores[0]['store_name']." already saved!");
	...
	$arrSearchStore = [];
	$arrSearchStoreGroup = [];
	foreach ($_POST['store_code_percentage_above'] as $key => $value) {
	...
	foreach ($arrStoresDataSearch as $key => $value) {
		...
		$query = 	'INSERT IGNORE INTO
					...
					VALUES (
						...
						"'.mysqli_real_escape_string($conn,$_POST['sram_above_percentage']).'",
						"'.mysqli_real_escape_string($conn,$_POST['am_above_percentage']).'",
						"'.mysqli_real_escape_string($conn,$_POST['suparea_above_percentage']).'",
						"'.mysqli_real_escape_string($conn,$_POST['staff_above_percentage']).'",
