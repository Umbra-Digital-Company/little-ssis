<?php
	...
	foreach($_FILES as $file) {
	  ...
	  foreach($content as $line){
	    ...
	    if($row ==1){
	    	if(trim(strtolower($data_row[0])) != 'product code' || (trim(strtolower($data_row[1])) != 'physical count' && trim(strtolower($data_row[1])) != 'count') ){
	    	$invalid = true;
	      ...
	      elseif(!is_numeric(trim($data_row[1]))){
	      	if($invalid == false){
		      	$invalid = true;
		      	$invalid_message = "Invalid physical count on line ".$row;
		    }else{
			    $invalid_message .=", ".$row;
			}
	      }
	      if(!array_filter($data_row)) break;
        	...
        	$arrData ['physical_count'] = trim($data_row[1]);
        	$data[] = $arrData;
	    }
	    $row++;
	  }
