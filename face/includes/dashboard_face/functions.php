...
$removeStoreIDs = "AND o.store_id NOT IN ('142', '1000', '900', '991', '126', '130', '787', '788','150', '999','139','155','0')";
$removeStoreLocations = "sl.store_code NOT IN ('142', '1000', '900', '991', '126', '130', '787', '788','150', '999','139','155','0')";
				...
				FROM 
					orders_face o
						...
						LEFT JOIN poll_51_face p51
							ON p51.item_code = os.product_code	
				WHERE					
					".$qDate."
					".$specStore."
					".$removeStoreIDs."
...
function grabItemsSoldSpecs() {
	...
	$arrBestFrame = array();
	 $query = 	"SELECT  
				    ...
				    0 AS 'count_with_prescription',
				    0 AS 'count_without_prescription',
				    0 AS 'count_lens_only',
				    0 AS 'count_single_vision_stock',
				    0 AS 'count_single_vision_rx',
				    0 AS 'count_double_vision_stock',
				    0 AS 'count_double_vision_rx',
				    0 AS 'count_progressive_stock',
				    0 AS 'count_progressive_rx',
				    0 AS 'count_special_order',
				    0 AS 'count_none',
					0 AS 'count_special_sv',

					0 AS 'count_special_dv',
					0 AS 'count_special_px',  
				    0 AS 'count_essilor',
				    0 AS 'count_housebrand',
				    0 AS 'sum_essilor',
					0 AS 'sum_housebrand'
				FROM 
					orders_face o
				...
				FROM 
					orders_face o
						LEFT JOIN orders_face_details os
							ON os.order_id = o.order_id
						LEFT JOIN poll_51_face p51
							ON p51.item_code = os.product_upgrade
						LEFT JOIN poll_51_face p51_studios
							ON p51_studios.item_code = os.product_upgrade
						LEFT JOIN poll_51_face p51_studios2
							ON p51_studios.item_code = os.product_code
				WHERE
					".$qDate."
						".$specStore."
						".$removeStoreIDs."
						AND os.status IN ('complete', 'dispatched', 'paid', 'received')
						AND os.product_code = 'M100' AND os.dispatch_type != 'packaging'		
						AND os.payment = 'y'
				GROUP BY
					os.product_upgrade						
					ORDER BY
	...
	$query = 	"SELECT  
				...
				WHERE
					".$qDate."
						...
						".$removeStoreIDs."
						AND os.status IN ('complete', 'dispatched', 'paid', 'received')
						AND os.product_upgrade = 'G100' AND os.dispatch_type != 'packaging'				
						AND os.payment = 'y'
				GROUP BY
					os.product_code						
					ORDER BY
	...
	$query = 	"SELECT  
				...
				WHERE
					...
					".$removeStoreIDs."
						AND os.payment = 'y'					
						AND os.status IN ('dispatched', 'received', 'paid', 'complete')
						AND os.product_upgrade <> 'sunnies_face'						
						AND os.lens_code != ''
						AND os.product_code <> 'M100'
						AND os.product_code <> 'S100'
						AND os.dispatch_type!='packaging'	
	...
	$query = 	"SELECT  
				...
				FROM 
					orders_face o
						...
						LEFT JOIN poll_51_face p51
							ON p51.item_code = os.product_upgrade
						LEFT JOIN poll_51_face p512
							ON p512.item_code = os.product_code
							LEFT JOIN poll_51_face p513
							ON p513.item_code = os.product_upgrade
				WHERE
					".$qDate."
					".$specStore."
					".$removeStoreIDs."
						...
						AND (
							os.product_code = 'M100'
								OR (
									os.product_upgrade = 'sunnies_face'
...
function grabRevenueBreakdown() {
	...
	if(!empty($arrFilterStores)) {
		...
		// Set WHERE query for stores
		$specStore = "AND (";
		$specStoreB = "AND sl.store_code IN (";		
	...
	if((isset($_GET['date']) && $_GET['date'] != 'custom') || $customMonth) {
		...
		$query .= 	"					AND os.status IN ('dispatched', 'received', 'paid', 'complete')
								...
								ON q.store_id_order = sl.store_code
					WHERE
	...
	if (mysqli_stmt_prepare($stmt, $query)) {
		...
		while (mysqli_stmt_fetch($stmt)) {
			...
			$tempArray['store_name'] = utf8_encode($tempArray['store_name']);
			$arrRevenue[] = $tempArray;
	...
	for ($i=0; $i < sizeOf($arrRevenue); $i++) { 
		...
		echo 				'<a class="col-10" style="text-decoration: underline !important;" target="_BLANK" href="/face/dashboard/face/?filterStores%5B%5D='.$curStoreID.'&'.$date_filter.'">';
	...
	$arrCustomerInfo = array();
	 ...
	 $query = 	"SELECT
					...
					CASE when pi.email_address 
							like '%guest@%' AND pi.email_address like '%sunniesstudios.com%'
								then 'guest'
					    when pi.email_address NOT LIKE '%guest@%' AND pi.email_address NOT LIKE '%sunniesstudios.com%' AND q.order_id <> os.order_id then 'recurring'
						when 
						pi.email_address
						NOT LIKE '%guest@%' AND pi.email_address NOT LIKE '%sunniesstudios.com%' AND q.order_id = os.order_id  then 'new'
						ELSE 'new'
					END
				FROM
					orders_face o
...
function grabGuestTable(){
	...
	$query ="SELECT 
					...
					FROM orders_face o 
						...
						LEFT JOIN poll_51_face p51 ON p51.item_code = os.product_code
						...
						WHERE
							...
							AND os.orders_specs_id!=''
							AND os.dispatch_type != 'packaging'
							AND pi.email_address LIKE '%guest@%' AND pi.email_address LIKE '%sunniesface.com%'
