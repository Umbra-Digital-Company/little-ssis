...
if(isset($_GET['search']) && $_GET['search'] != '') {
	...
	$querySearch .= " AND os.date_created > DATE_ADD(NOW(), INTERVAL -1 WEEK)";
};
...
$querypn = 	"SELECT 
				...
				p.profile_id,
				os.status,
				DATE_ADD(os.status_date, INTERVAL 12 HOUR),
				os.lens_option,
			...
			FROM 
				profiles_info p
					...
					LEFT JOIN 
							poll_51_face ps 
							ON  ps.item_code=os.product_code
					LEFT JOIN 
							poll_51 psp
							ON  psp.product_code=os.product_code
					LEFT JOIN 
							poll_51_face psm
							ON  psm.item_code=os.product_upgrade
					LEFT JOIN 
...
if (mysqli_stmt_prepare($stmt, $query2)) {
	...
	mysqli_stmt_bind_result($stmt, $result1, $result2, $result3, $result4, $result5, $result6, $result7, $result8, $result9,
	 ...
	 ,$result33,$result34,$result35,$result36,$result37,$result38,$result39,$result40,$result41,$result42,$result43);
