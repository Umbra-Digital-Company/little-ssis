...
$page = 'store';
$page_url = 'inventory-lookup-face';

$filter_page = 'inventory_lookup_store_face';
$group_name = 'aim_face';
...
if(isset($_GET['frame_code'])){
    ...
    for ($i=0;$i<sizeof($arrStoresFace);$i++) {
		$store_name=array_push($branches,$arrStoresFace[$i]["store_id"]);
		$store_array=array_push($Names_branch,$arrStoresFace[$i]["store_name"]);
		$storeNo_array=array_push($phone_branch,"none");
        $FrameData[$_GET['frame_code']][$arrStoresFace[$i]["store_id"]]= storeChecker_smr($_GET['frame_code'],$arrStoresFace[$i]["store_id"],$dateStart,$dateEnd);
	...
	<div id="ssis-main" class="col <?= str_replace(' ','-',$page_url) ?>">
		...
		<div class="ssis-content">
			...
			<div id="inventory-receive" style="max-width: 768px">
				...
				<form action="/face/inventory/store/inventory-lookup-face/" method="GET" id="request-form">
