...
require $sDocRoot."/face/inventory/includes/s_pd_functionv2.php";
...
$user_type_d=$_SESSION['user_login']['position'];

if($_GET['branch']=='store'){
	if($_GET['column_header']=='DAILY SALES'){
		if($_GET['filterStores']=='788'|| $_GET['filterStores']=='787'){

			$GetData=	perdayStoreSalesVS($_GET['filterStores'],$_GET['sku_code'],$_GET['dateStart'],$_GET['dateEnd'],'store');
		...
		|| $_GET['filterStores']=='6008055'   || $_GET['filterStores']=='SS-MPWHC' || $_GET['filterStores']=='6008058'|| $_GET['filterStores']=='6008059'
			...
			$GetData=	perdayStoreSalesMS('SS-MPWHC',$_GET['sku_code'],$_GET['dateStart'],$_GET['dateEnd'],'store');
		}else{
...
function GetName($store_id){
	...
	if($storelength=='3' || $storelength=='7'){
		$query="SELECT store_name_proper FROM store_codes_face where store_code='".$store_id."' ";
	}elseif($store_id=='SS-MPWHC'){
...
function GetInfo($store_id){
  ...
  $type_filter=GetInfo($_GET['filterStores']);
?><style>
...
<?php if($_GET['column_header']=='DAILY SALES'){ ?>
				<table class="table-striped table-inventory" style="width:150%">
				<tr>
				<th >#</th>
...
<?php if($_GET['column_header']=='STOCK TRANSFER (+)' || $_GET['column_header']=='INTER BRANCH (+)' || $_GET['column_header']=='PULLOUT' || $_GET['column_header']=='DAMAGE' || $_GET['column_header']=='IN TRANSIT(+)'  ){ ?>
				...
				<?php if($GetData){ ?>
					<?php for($i=0;$i<sizeof($GetData);$i++){?>
						...
						?></td>
							<td><?php if($user_type_d=='store'){?>
								<a href='/face/inventory/store/face-history/?ref_num=<?=  $GetData[$i]["reference_number"]; ?>'  target='_blank'><?= $GetData[$i]["reference_number"]; ?> </a>
								<?php
						}else{ ?>

						<a href='/face/inventory/admin/face-history-all/?ref_num=<?= $GetData[$i]["reference_number"]; ?>' target='_blank'>	<?= $GetData[$i]["reference_number"]; ?> </a>
