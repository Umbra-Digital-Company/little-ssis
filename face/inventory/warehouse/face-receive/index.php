...
$page = 'warehouse';
$page_url = 'receive-face';

$filter_page = 'receive_warehouse_face';
$group_name = 'aim_face';
////////////////////////////////////////////////
...
require $sDocRoot."/includes/get_full_name.php";
	...
	<div id="ssis-main" class="col <?= str_replace(' ','-',$page_url) ?>">
		...
		<?php if($_SESSION["store_code"]=='warehouse_qa'){?>
			...
			<div id="inventory-receive">
				...
				<form action="/face/inventory/process/warehouse/face_transfer.php" method="POST" id="pullout-form" autocomplete="off">
					<div class="row align-items-start">
						...
						<div class="col-12 col-lg-4">
							<div class="custom-card">
								<form class="form-horizontal form-csv" action="" method="POST" name="uploadCSV" enctype="multipart/form-data">
									<div class="form-group">
										...
										<p class="text-secondary">Import a .CSV file with two columns. The header of the first column should be "Product Code" and the second column should be "Transferred Count" and the third column should be "Received Count".</p>
									</div>
									<div class="form-group">
							...
							<div class="mt-4">
								...
								<p class="h6 large"><?= ($arrReceivable[0]['transaction_reason'] != '') ? $arrReceivable[0]['transaction_reason'] : '-' ?></p>
							</div>
							<div class="mt-3">
								<h5 class="text-secondary font-bold">Employee Name</h5>
								<input type="hidden" name="emp_id" value="<?= $_SESSION['user_login']['store_code'] ?>">
								<input type="text" name="emp_name" class="form-control" placeholder="Full name" value="<?= getFullName() ?>">
							</div>
							<div class="mt-3">
							...
							<div class="mt-5">
								<div class="d-flex justify-content-end">
									<a href="/face/inventory/warehouse/face-receive/" class="mr-3">
		...
		<?php } else { ?>
			...
			?>
				<form action="/face/inventory/process/warehouse/face_receive.php" method="POST" id="receive-form">
					<div class="row align-items-start">
						<div class="col-12 col-lg-8">
							<div class="custom-card">
								<div class="row">
							...
							<div class="mt-5">
								<div class="d-flex justify-content-end">
									<?php if ( $arrReceivableItems[0]['status']=='in transit') { ?>
										<button type="submit" class="btn btn-primary" id="confirmForm" disabled>receive</button>
									<?php } ?>									
									<a href="/face/inventory/warehouse/face-receive/" class="ml-3"><button type="button" class="btn btn-secondary">Exit</button></a>
								</div>
							</div>
								...
								<tbody>
									...
									<?php for ( $i=0; $i<sizeof($arrReceivable); $i++ ) { 
										...
										<tr>
											...
											<td nowrap class=""><a href="/face/inventory/warehouse/face-receive/?id=<?= $arrReceivable[$i]['reference_number'] ?>" class="text-success text-uppercase font-weight-bold"><?= $arrReceivable[$i]['status']=='in transit' ? 'receive' : 'view' ?></a></td>
										</tr>
...
<script>
	...
	$(document).ready(function() {
	    ...
	    function trNewLine(code, transffered_count, received_count){
			let newLine =  '<tr>'
						+'<th style="width: 360px;">'
							+'<select name="frame_code[]" class="select2 filled frame_code" style="width: 336px;" first_line_required >'
								...
								textProductColor = '<?= addslashes(ucwords(strtolower(str_replace("-", " ", $arrItems[$i]['product_style'] . $arrItems[$i]['product_color'])))) ?>';
								newLine += '<option value="<?= $arrItems[$i]['product_code'] ?>" '+selected+'>'+textProductColor+' ( <?= $arrItems[$i]['product_code'] ?> )</option>'
						...
						+'</th>'
						+'<td style="min-width:100px;max-width:100px;">'
							+'<input min="0" type="number" name="frame_count[]" class="form-control filled" placeholder="#" value="'+transffered_count+'" first_line_required>'
						+'</td>'
						+'<td style="min-width:100px;max-width:100px;">'
							+'<input min="0" type="number" name="received_count[]" class="form-control filled" placeholder="#" value="'+received_count+'" first_line_required>'
						+'</td>'
				...
				transFerCount = parseInt($('#total-transfer-calc').text());
				receivedCount = parseInt($('#total-receive-calc').text());
				transFerCount += parseInt(transffered_count);
				receivedCount += parseInt(received_count);

				$('#total-transfer-calc').text(transFerCount);
				$('#total-receive-calc').text(receivedCount);
			return newLine;
		}
	    $('#upload_file').on('click', function(e) {
	        ...
	        if($('input[type=file]').val() == '') {
	            ...
	            // Check file size
	                ...
	                $.ajax({
	                    url: '/process/upload_physical_count_qa.php',
	                    type: 'POST',
	                    data: data,
	                    cache: false,
	                    dataType: 'json',
	                    ...
	                    success: function(response, textStatus, jqXHR){
	                    	...
	                    	if(response.invalid_message != false){
	                    		...
	                    		$('#total-transfer-calc').text(0);
								$('#total-receive-calc').text(0);
