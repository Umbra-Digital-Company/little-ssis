    ...
    $arrInvLook=array();
             $query=" SELECT 
                ...
                'warehouse',
                        ...
                        from inventory_face   iib
                    WHERE
                    iib.product_code =p51.item_code
                    AND
                ...
                (select coalesce(
                        ...
                        ),0) from inventory_face   iip
                                WHERE
                                iip.product_code =p51.item_code
                                AND
                ...
                (select coalesce(
                        ...
                        ),0) from inventory_face  iid
                                WHERE
                                iid.product_code =p51.item_code
                                AND
                    ...
                    (select coalesce(
                        ...
                        ),0) from inventory_face  iiso
                                WHERE
                                iiso.product_code =p51.item_code
                                AND
                    ...
                    (select coalesce(
                        ...
                        '0',
                'none',(SELECT coalesce(sum(REPLACE(count,',','')),0) from inventory_face  ito
                                                    WHERE
							...
							AND
									...
									(
                                        ...
                                        SELECT COALESCE(sum(iaccc.input_count),0)- COALESCE(sum(iaccc.running),0)
																	from inventory_actual_count_face  iaccc 
																	where iaccc.date_end<'".$date_start."'  and iaccc.store_audited='warehouse'
																	and iaccc.product_code='".$product_Code."'
																	and iaccc.date_end=(SELECT max(iacx.date_end) from inventory_actual_count_face  iacx 
									...
									(
                                        ...
                                        SELECT COALESCE( max(iaccc.date_end),0)
                                                            ...
                                                            ,
                                                                    ...
                                                                    AND
                                                                        DATE(DATE_ADD(iib.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx.status_date), INTERVAL 13 HOUR)) from inventory_face  iacx 
                                                                                                                WHERE iacx.product_code='".$product_Code."'
        ...
        (select  DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) from inventory_face  iid
                WHERE
                iid.product_code =p51.item_code
                AND
                iid.stock_from='warehouse'
            ...
            AND iid.type='damage'
            AND
            DATE(DATE_ADD(iid.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx.status_date), INTERVAL 13 HOUR)) from inventory_face  iacx 
                                                                                    WHERE iacx.product_code='".$product_Code."'
        ...
        (select  DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) from inventory_face   iip
                            WHERE
                            iip.product_code =p51.item_code
                            AND
                            iip.stock_from='warehouse'
                        ...
                        AND iip.type='pullout'
                        AND
                        DATE(DATE_ADD(iip.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx.status_date), INTERVAL 13 HOUR)) from inventory_face  iacx 
                                                                                    WHERE iacx.product_code='".$product_Code."'
                                                                                AND iacx.stock_from='warehouse'
                                                                                AND  DATE(DATE_ADD(iacx.status_date, INTERVAL 13 HOUR))<'".$date_start."' 
                                                                                AND count !='0'
                        ...
                        (select   DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) from inventory_face   iiiboc
                                WHERE
                                iiiboc.product_code =p51.item_code
                                AND
                                iiiboc.stock_from='warehouse'
                            ...
                            AND
                               ...
                               (select DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) from inventory_face  iinbic
                                       WHERE
                                       iinbic.product_code =p51.item_code
                                       AND
                                       iinbic.store_id='warehouse'
            ...
            FROM    poll_51_face p51
            LEFT JOIN inventory_face ip on p51.item_code=ip.product_code
...
function storeChecker_smr($product_code,$store_id,$date_start,$date_end){
    ...
    // if($store_id=='787' ||  $store_id=='788'){
        ...
        || preg_match("/PL/i", $product_code) || preg_match("/60319/i",$product_code)
                            ...
                            || preg_match("/MSCL/i",$product_code)  || preg_match("/SDB/i",$product_code)  || preg_match("/SSWP/i",$product_code)  || preg_match("/SWB/i",$product_code)   || preg_match("/SC/i",$product_code) || preg_match("/GRH/i",$product_code) || preg_match("/MSAC/i",$product_code)  || preg_match("/MSKC/i",$product_code)  || preg_match("/KLB/i",$product_code)  || preg_match("/MSEP/i",$product_code)  || preg_match("/SPB0/i",$product_code)  || preg_match("/MSLS/i",$product_code)
                               || preg_match("/MSHS/i",$product_code) || preg_match("/SSGWPCB/i",$product_code) || preg_match("/SSP/i",$product_code)  
                               || preg_match("/HT0/i",$product_code)  || preg_match("/TB0/i",$product_code)  || preg_match("/SGC/i",$product_code)  
                              || preg_match("/DS/i",$product_code)   || preg_match("/ST/i",$product_code)   || preg_match("/SMS/i",$product_code) 
                               || preg_match("/SML/i",$product_code)  || preg_match("/CPV/i",$product_code)  
                              || preg_match("/SWS/i",$product_code)   || preg_match("/SMHP/i",$product_code) 
                              || preg_match("/SFSP/i",$product_code) || preg_match("/MSTS/i",$product_code) 
                              || preg_match("/SMZ/i",$product_code) 
                              ){
              ...
              $condition1=" and ( os.product_upgrade ='sunnies_face'  OR   os.product_upgrade ='G100')";
              $carekits=" AND os.product_code ";
     ...
     $query ="SELECT 
        ...
        (select  DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) from inventory_face   iip
                        ...
                        DATE(DATE_ADD(iip.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx.status_date), INTERVAL 13 HOUR)) from inventory_face  iacx 
                            ...
                            FROM poll_51_face p51
                            LEFT JOin inventory_face ip on ip.product_code=p51.item_code
                            LEFT JOIN store_codes_face sls on sls.store_code='".$store_id."'
                                
                                WHERE p51.item_code='".$product_code."'
                                AND  sls.store_code   ='".$store_id."'
                                group by sls.store_code,p51.item_code
                                ";
...
function storeChecker_smr_MS($product_code,$store_id,$date_start,$date_end){
    ...
    // }
      ...
      $stock_from = "  AND os.stock_from='SF-MPWHC'  ";
  ...
  $query ="SELECT 
        ...
        (select  DATE(DATE_ADD(  max(status_date), INTERVAL 13 HOUR)) from inventory_face   iip
                        ...
                        DATE(DATE_ADD(iip.status_date, INTERVAL 13 HOUR))=(SELECT  DATE(DATE_ADD(max(iacx.status_date), INTERVAL 13 HOUR)) from inventory_face  iacx 
                            ...
                            FROM poll_51_face p51
                            LEFT JOin inventory_face ip on ip.product_code='".$product_code."'
                            LEFT JOIN labs_locations sls on sls.lab_id='".$store_id."'
                                
                                WHERE p51.item_code='".$product_code."'
                                AND  sls.lab_id   ='".$store_id."'
                              
                                group by sls.lab_id,p51.item_code
                                ";
