			...
			<div class="order-details">
				...
				<div class="row align-items-start">
								...
								<table class="table table-striped table-inventory mb-0">
									...
									<tbody>
										...
										<?php for ($i=0; $i < sizeOf($arrReceivable); $i++) {
											...
											if($arrReceivable[$i]['runner_count'] < $arrReceivable[$i]['count']) {
											...
											echo 	'<td style="vertical-align:middle!important;" class="text-center '.$styleRed.'">'.$arrReceivable[$i]['runner_count'].'</td>';
											// Check if actual count is lower
									...
									<tfoot>
										<tr>
											...
											<th class="small text-white text-uppercase text-center"><?= array_sum(array_column($arrReceivable, 'runner_count')); ?></th>
					...
					<div class="col-12 col-lg-4 mt-5 mt-lg-0">
						...
						<?php } ?>
						<div class="mt-3">
							<h5 class="text-secondary font-bold">Reason Remarks</h5>
						...
						<div class="mt-3">
							...
							<p class="h6 large">
								<?php if ($arrReceivable[0]['admin_id']=='overseer') {
									echo 'Admin - ' . ucwords(strtolower($arrReceivable[0]['admin_name']));
								} elseif ($arrReceivable[0]['sender_id']=='warehouse') {
									echo 'Warehouse - ' . ucwords(strtolower($arrReceivable[0]['sender_name']));
								} elseif ($arrReceivable[0]['sender_id']=='warehouse_qa') {
									echo 'Warehouse QA - ' . ucwords(strtolower($arrReceivable[0]['sender_name']));
								} else {
						...
						<div class="mt-3">
							...
							<div class="custom-card mt-3">
								<?php if ( $arrReceivable[0]['signature'] != '' ) { 
									$senderSignature = $arrReceivable[0]['signature'];
								} elseif ( $arrReceivable[0]['admin_signature'] != '' ) {
									$senderSignature = $arrReceivable[0]['admin_signature'];
								}?>
								<img class="img-fluid center-block" src="<?= $senderSignature ?>">
							</div>
						</div>
							...
							<div class="mt-3">
								...
								<p class="h6 large">
									<?php if ($arrReceivable[0]['receiver_id']=='overseer') {
										echo 'Admin - ' . ucwords(strtolower($arrReceivable[0]['receiver_name']));
									} elseif ($arrReceivable[0]['receiver_id']=='warehouse') {
										echo 'Warehouse - ' . ucwords(strtolower($arrReceivable[0]['receiver_name']));
									} elseif ($arrReceivable[0]['receiver_id']=='warehouse_qa') {
										echo 'Warehouse QA - ' . ucwords(strtolower($arrReceivable[0]['receiver_name']));
									} else {
										echo ucwords(strtolower($arrReceivable[0]['receiver_first_name'].' '.$arrReceivable[0]['receiver_last_name']));
									} ?>
								</p>
							</div>
							<div class="mt-3">
