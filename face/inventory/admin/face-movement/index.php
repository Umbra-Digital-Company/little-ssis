...
$filter_page = 'stock_movement_admin_face';
$group_name = 'aim_face';
...
if(isset($_GET['filterStores'])){
	...
	elseif($_GET['filterStores']=='5000470' || $_GET['filterStores']=='5000472' || $_GET['filterStores']=='5000473'  || $_GET['filterStores']=='6008053'  || $_GET['filterStores']=='6008054'
		...
		|| $_GET['filterStores']=='6008060'|| $_GET['filterStores']=='5000476'|| $_GET['filterStores']=='5000477'|| $_GET['filterStores']=='5000478'){
			$branchName ="SS MarketPlace";
		}
	else{
		$MergeStores=array_merge($arrStore, $arrStudiosStore);
					...
					for ($i=0; $i < sizeOf($MergeStores); $i++) { 
						...
						if($MergeStores[$i]['store_id'] ==$_GET['filterStores'] ) {

							$branchName = $MergeStores[$i]['store_name'];
...
if(isset($_GET['data_range_start_year'])){
    $audit_date_check=$_GET['data_range_start_year']."-".$_GET['data_range_start_month']."-".$_GET['data_range_start_day'];
}else{
    $audit_date_check= date("Y-m-d");
}


$queryMaxDate="SELECT Max(date_end),max(date_start) FROM inventory_actual_count_face
where  store_audited='".$store_id."'
AND date_end< '".$audit_date_check."'  ";

$grabInvParams2=array("max_date","min_date");
...
if(isset($_GET['date'])){
	if($_GET['date']=='month'){
		...
		$dateStartpdh = ddate("Y-m-d", strtotime($arrMaxDate2[0]["max_date"] . " +1 day"));;
		$dateEndpdh = date('Y-m-t');
	}
	elseif($_GET['date']=='yesterday'){
		$datecurrentH = date('Y-m',strtotime("-1 days"));
	 	$dateStartpdh = date("Y-m-d", strtotime($arrMaxDate2[0]["max_date"] . " +1 day"));;
	 	$dateEndpdh = date('Y-m-t');
	...
	$datecurrentH = date('Y-m');
	$dateStartpdh =	$dateStartpdh =date("Y-m-d", strtotime($arrMaxDate2[0]["max_date"] . " +1 day"));;
...
if(isset($_GET['date'])){
	if($_GET['date']=='month'){
		$dateStart = date("Y-m-d", strtotime($arrMaxDate2[0]["max_date"] . " +1 day"));;
		$dateEnd= date('Y-m-t');
	}
	elseif($_GET['date']=='yesterday'){
        $dateStart = date("Y-m-d", strtotime($arrMaxDate2[0]["max_date"] . " +1 day"));;
...
$date_title =cvdate(3,$dateStart)." - ".cvdate(3,$dateEnd);
?>
		...
		<div class="ssis-content">
				...
				<form method="GET" class="filter-body">
								...
								<select name="filterStores" id="filterStores" class="select2 form-control">
									...
									<optgroup label="STORE NAME">
										<?php for ($i=0;$i<sizeof($arrStoresFace);$i++) { ?>
											<option value="<?= $arrStoresFace[$i]['store_id'] ?>" <?=$branchtype==$arrStoresFace[$i]['store_id'] ? 'selected' : '' ?>><?= ucwords(str_replace(['ali','sm','mw'],['ALI','SM','MW'],strtolower($arrStoresFace[$i]['store_name']))) ?></option>
										<?php } ?>
									</optgroup>
							...
							<div class="col-12 col-md-6">
								...
								<?php
									...
									if(isset($_GET['data_range_start_month'])) {
										...
										$sStartMonth = date('F',strtotime(date("Y-m-d", strtotime($arrMaxDate2[0]["max_date"] . " +1 day"))));
										$sStartDay 	 =  date('d',strtotime(date("Y-m-d", strtotime($arrMaxDate2[0]["max_date"] . " +1 day"))));
										$sStartYear  =date('Y',strtotime(date("Y-m-d", strtotime($arrMaxDate2[0]["max_date"] . " +1 day"))));
									};
									...
									if(isset($_GET['data_range_end_month'])) {
										...
										$sStartMonth = date('F',strtotime($arrMaxDate2[0]["max_date"] . " +1 day"));
										$sStartDay 	 =  date('d',strtotime($arrMaxDate2[0]["max_date"] . " +1 day"));
										$sStartYear  =date('Y',strtotime($arrMaxDate2[0]["max_date"] . " +1 day"));
					...
					<div class="text-center mt-5 pb-5">
						...
						<?php if(isset($_GET['date'])) { ?>
							...
							<a href="/studios/inventory/admin/studios-movement" class="ml-0 mt-3 mt-sm-0 mt-sm-3 d-block d-sm-inline d-lg-none">
			...
			<div class="d-flex no-gutters align-items-center" id="data-filter">
				...
				<?php if(isset($_GET['date'])) { ?>
					...
					<a href="/studios/inventory/admin/studios-movement" class="d-none d-lg-block text-danger text-uppercase font-bold">
						reset filter
					</a>
			...
			?><div class="input-group-prepend">
			Previous AUDIT  Start Date is : <?= $arrMaxDate2[0]["min_date"]  ?>
			  </div>
				<div class="input-group-prepend">
			    Previous AUDIT  END Date is : <?= $arrMaxDate2[0]["max_date"]  ?>
			  	</div>
			</div>
...
for ($i=0;$i<sizeof($arrPoll51_items);$i++) {
								...
								?>
								<tr class="row100 body" filterStores="<?= $_GET['filterStores'] ?>" product_code="<?= $arrPoll51_items[$i]["product_code"] ?>" dateStart="<?= $dateStart ?>" dateEnd="<?= $dateEnd ?>" datecurrentH ="<?= $datecurrentH ?>" dateStartpdh ="<?= $dateStartpdh ?>" dateEndpdh ="<?= $dateEndpdh ?>">
						...
						<section>
							<div class="download_section">
								...
								<div class="dl_btn input-group-prepend">
							    	<span class="btn btn-primary text-white input-group-text" id="btn_download" style="display: none;">Download</span>
							  	</div>
							  	<span style="float: right;" class="loading_all"><img src="/images/loading_gray.gif" width="30px" height="30px"></span>
							</div>
						</section>
					</div>
				</div>
					...
					<div class="d-flex align-items-center">
						<section>
							<div class="download_section">
								...
								<div class="dl_btn input-group-prepend">
							    	<span class="btn btn-primary text-white input-group-text" id="btn_download_all" style="display: none;">Download</span>
							  	</div>
							  	<span style="float: right;" class="loading_all"><img src="/images/loading_gray.gif" width="30px" height="30px"></span>
							</div>
						</section>
