...
$page = 'audit';
$page_url = 'face-physical-count';

$filter_page = 'phyiscal_count_audit_face';
$group_name = 'aim_face';
	...
	if($_GET['filterStores']=='warehouse'){
		...
		$MergeStores= $arrStoresFace;
					...
					for ($i=0; $i < sizeOf($MergeStores); $i++) { 
						...
						if($MergeStores[$i]['store_id'] ==$_GET['filterStores'] &&  $_GET['filterStores']!='SF-MPWHC' ) {
							$branchType = 'store';
							$branchName = $MergeStores[$i]['store_name'];

						}elseif($_GET['filterStores']=='SF-MPWHC'){
							$branchType = 'store';
			...
			<div class="row" style="max-width: 100%;">
 				...
 				<div class="col-12 col-lg-8" style="overflow:hidden">
					...
					<div id="excel-inventory" class="custom-card p-0">
						<form action="/face/inventory/process/face_actual_count.php" method="POST" autocomplete="off" class="table-default table-responsive" style="max-width: 100%;">
								...
								<select name="filterStores" id="id" class="select2 form-control" required>
									...
									<optgroup label="STORE NAME">
										<?php for ($i=0;$i<sizeof($arrStoresFace);$i++) { ?>
											<option value="<?= $arrStoresFace[$i]['store_id'] ?>" <?= isset($_GET['filterStores']) && $_GET['filterStores']==$arrStoresFace[$i]['store_id'] ? 'selected' : '' ?>><?= ucwords(str_replace(['ali','sm','mw'],['ALI','SM','MW'],strtolower($arrStoresFace[$i]['store_name']))) ?></option>
										<?php } ?>
									</optgroup>
