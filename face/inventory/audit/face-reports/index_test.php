...
$filter_page = 'reports_audit_face';
$group_name = 'aim_face';
...
require $sDocRoot."/face/inventory/includes/grab_all_auditor_reports_face.php";
// 2.0
				...
				<?php if ( !empty($arrVarianceReport) ) { ?>
					<div style="display: flex; justify-content: flex-end;">
						<div class ="col-md-3 col-xs-12 input-group" style="padding-right: 0px;">
							...
							<select class="form-control" name="branch_search" id="branch_search">
                                ...
                                <?php
                                    ...
                                    if(isset($_GET['search'])){
                                            $store_id=$_GET['search'];

                                    }else{
                                            $store_id="";
                                    }

                                    // echo '<optgroup label="SPECS STORES">';
                                    // for ($i=0; $i < sizeOf($arrStores); $i++) { 
                                    //     $curStoreName = ucwords(str_replace("u.p.", "UP", str_replace("sm", "SM", str_replace("-", " ", $arrStores[$i]['store_name']))));
                                    //     echo '<option value="'.$arrStores[$i]['store_id'].'" '.( ($store_id == $arrStores[$i]['store_id'] ? "selected" : "") ).'>'.$curStoreName.'</option>';	
                                    // };
                                    // echo '</optgroup>';
                                    // echo '<optgroup label="STUDIOS STORES">';
                                    // for ($i=0; $i < sizeOf($arrStoresStudios); $i++) { 
                                    //     echo '<option value="'.$arrStoresStudios[$i]['store_id'].'" '.( ($store_id == $arrStoresStudios[$i]['store_id'] ? "selected" : "") ).'>'.$arrStoresStudios[$i]['store_name'].'</option>';	
                                    // };
                                    // echo '</optgroup>';
                                    // echo '<optgroup label="FACE STORES">';
                                    for ($i=0; $i < sizeOf($arrStoresFace); $i++) { 
                                        echo '<option value="'.$arrStoresFace[$i]['store_id'].'" '.( ($store_id == $arrStoresFace[$i]['store_id'] ? "selected" : "") ).'>'.$arrStoresFace[$i]['store_name'].'</option>';	
                                    };
                                    // echo '</optgroup>';
						...
						<table class="table table-striped mb-0">
							...
							<?php for($i=0;$i<sizeof($arrVarianceReport);$i++){ ?>
								<tr>
									...
									<td nowrap class=""><a href="/face/inventory/audit/face-reports/view/?id=<?= $arrVarianceReport[$i]['branch_id'] ?>&date=<?= $arrVarianceReport[$i]['date_end'] ?>" class="text-primary text-uppercase font-weight-bold">view</a></td>
								</tr>
							<?php } ?>
							</tbody>
						</table>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('#btn_search').on('click', function(){
			...
			window.location ='/face/inventory/audit/face-reports/index_test.php'+search;
		});
