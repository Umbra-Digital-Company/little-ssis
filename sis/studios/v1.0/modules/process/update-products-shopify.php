...
for ($i=0; $i < sizeOf($arr_products_sorted); $i++) { 
	...
	for ($a=0; $a < sizeOf($curDataArray); $a++) { 
		...
		$query  =   "INSERT INTO 
		            ...
		            VALUES (
		            	'".mysqli_real_escape_string($conn, $curDataArray[$a]["sku"])."',
		            	'".mysqli_real_escape_string($conn, $curDataArray[$a]["id"])."',
		            	'".mysqli_real_escape_string($conn, $curDataArray[$a]["product_id"])."',
		            	'".mysqli_real_escape_string($conn, $curDataArray[$a]["product_type"])."',
		            	'".mysqli_real_escape_string($conn, $curDataArray[$a]["image_url"])."',
		            	'".mysqli_real_escape_string($conn, $curDataArray[$a]["description"])."',
		            	'".mysqli_real_escape_string($conn, $curDataArray[$a]["tags"])."'
		        	)
