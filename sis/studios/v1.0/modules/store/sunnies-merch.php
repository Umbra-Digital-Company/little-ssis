                        ...
                        <div class="d-flex d-flex-m justify-content-between">
                            <div class="product-top" style="text-align:center;">
                                ...
                                <?php
                                    ...
                                    if($curImageURL == '/sis/studios/assets/images/defaults/no_specs_frame_available_b.png') {
                                ...
                                <div style="background: #e8e8e4; padding: 15px; border-radius: 0 0 10px 10px;">
                                    ...
                                    <ul class="row switch-color p-0" style="margin: 0 -3px;">
                                        ...
                                        <?php for($a = 0; $a< sizeOf($curColors); $a++) { ?>
                                            ...
                                            <li class="<?= $a === 0 ? 'active' : '' ?>" data-index="<?= $a ?>" data-style-name="<?= trim(str_replace(" ", "-", strtolower($arrProductsSorted[$i]['item_description']))) ?>" data-color-name="<?= trim($curColors[$a]['color']) ?>" data-color-code="<?= trim($curColors[$a]['product_code']) ?>" style="<?=($curColors[$a]['color_picker'] != '') ? $curColors[$a]['color_picker'] : 'background-color: #000;' ?>"></li>
                                    ...
                                    <div class="row d-flex justify-content-center mt-3">
                                        <form class="col-12 form-quick-add-to-bag" id="form-quick-add-to-bag<?= $i ?>" method="POST">
                                            <input type="hidden" name="studios_product_code" id="input-sku-<?= trim(str_replace(" ", "-", strtolower($arrProductsSorted[$i]['item_description']))) ?>" value="<?= trim($curColors[0]['product_code']) ?>">
...
<script>
    ...
    $(document).ready(function(){
        ...
        $(document).on('submit', "#form-add-to-bag", function(e){
            e.preventDefault();
            $.ajax({
                url: "/sis/studios/func/process/add_to_bag_merch.php",
                type: "POST",
                data: $(this).serialize(),
                dataType: 'html',
