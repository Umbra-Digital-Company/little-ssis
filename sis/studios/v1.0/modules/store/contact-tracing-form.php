...
<?php } else { ?>
		...
		<div class="switch-layout">
			...
			<div class="account-navigation d-flex no-gutters">
				<a href="#create-content"
					class="col-6 text-center account-option <?= (!isset($_SESSION['customer_id']) && !isset($_SESSION['temp_data'])) ? 'active' : '' ?>"><?= $arrTranslate['Register'] ?></a>
				<a href="#use-content"
					class="col-6 text-center account-option <?= (isset($_SESSION['customer_id']) || isset($_SESSION['temp_data'])) ? 'active' : '' ?>"><?= $arrTranslate['Log In'] ?></a>
			</div>
		</div>
	<?php endif ?>
		...
		<form method="post" id="use_account" name="use_account" autocomplete="off" class="mt-0">
			...
			<div class="d-flex form-row form-group justify-content-center mt-3">
				<div class="form-group col-md-2 col-sm-2" id="div_area_codes"
					...
					<select class="text-left select mh-40 form-control" name="country_codes_login" id="country_codes_login"
				...
				<div class="form-group col-md-12 col-sm-12" id="div_mobile_number">
					<input type="text" name="username" class="form-control text-lowercase" id="username" required="required"
						autocomplete="nope" />
					<label class="placeholder" for="username"
						style="margin-left:15px;"><?= $arrTranslate['Email or Mobile Number'] ?></label>
			...
			<div class="text-center mt-5">
				<div class="form-row">
					<div class="col-md-12 form-group">
						<input type="button" name="btnsubmit" id="btnsubmit" value="<?= $arrTranslate['Continue'] ?>"
	...
	<div class="account-content active mt-4 mb-4" id="create-content">
		<form name="create_account" id="create_account" action="/sis/studios/func/process/store-register-v2.php"
			...
			<p class="font-bold text-uppercase text-primary"><?= $arrTranslate['Profile Details'] ?></p>
			<div class="form-row no-gutters mt-3">
				...
				<div class="col-12 col-lg-16 col-md-6">
					<!-- <div class="form-group">
					...
					<input type="date" class="form-control sr-only" id="bdate2"
						value="<?= get_customer_data('birthday') ?>">
					<?php
					$birthDate = get_customer_data('birthday');
					$arrBirthDate = explode('-', $birthDate);
					?>

					<div class="d-flex form-row form-group justify-content-center">
						<div class="col-4 form-group">
							<select name="b_day" id="form-birthDay" class="form-control " required>

								<option value=""></option>
								<?php
								for ($i = 1; $i <= 31; $i++) {
									$i = ($i < 10) ? '0' . $i : $i;
									$selected = (isset($arrBirthDate[2]) && $i == $arrBirthDate[2]) ? 'selected' : '';
									?>
									<option value="<?= $i ?>" <?= $selected ?>><?= $i ?></option>
								<?php } ?>
							</select>
							<label class="placeholder" for="form-birthDay"><?= $arrTranslate['Birth Day'] ?></label>
						</div>
						<div class="col-4 form-group pl-0 pr-0">
							<select name="b_month" id="form-birthMonth" class="form-control" required>
								<option value=""></option>
								<?php

								$arrMonth = ['01' => 'January', '02' => 'February', '03' => 'March', '04' => 'April', '05' => 'May', '06' => 'June', '07' => 'July', '08' => 'August', '09' => 'September', '10' => 'October', '11' => 'November', '12' => 'December'];

								foreach ($arrMonth as $key => $value) {
									$selected = (isset($arrBirthDate[1]) && $key == $arrBirthDate[1]) ? 'selected' : '';

									?>
									<option value="<?= $key ?>" <?= $selected ?>><?= $value ?></option>
								<?php } ?>
							</select>
							<label class="placeholder" for="form-birthMonth"><?= $arrTranslate['Birth Month'] ?></label>
						</div>
						<div class="col-4 form-group">
							<select name="b_year" id="form-birthYear" class="form-control " required>
								<option value=""></option>
								<?php
								$year = date('Y');
								for ($i = $year; $i >= 1920; $i--) {
									$selected = (isset($arrBirthDate[0]) && $i == $arrBirthDate[0]) ? 'selected' : '';
									?>
									<option value="<?= $i ?>" <?= $selected ?>><?= $i ?></option>
								<?php } ?>
							</select>
							<label class="placeholder" for="form-birthYear"><?= $arrTranslate['Birth Year'] ?></label>
						</div>
					</div>
				...
				<div class="col-12 col-md-6">
					<div class="form-group gender">
						...
						<label class="placeholder" for="gender"><?= $arrTranslate['I Identify As?'] ?></label>
					</div>
				</div>
				<?php
			...
			<div class="form-row no-gutters mt-3">
				...
				<div class="col-md-12 col-sm-12 d-flex align-items-center">
					<div class="form-group col-md-2 col-sm-3" style='padding-left: 0px; padding-right: 0px;'>
						<select class="text-left select mh-40 form-control" name="country_codes" id="country_codes"
							style="margin: 0px; padding-top:0px; padding-bottom:0px; border-bottom-right-radius: 0px; border-top-right-radius: 0px;"
							required>
						...
						<label class="placeholder" for="country_codes"><?= $arrTranslate['Area Code'] ?></label>
					</div>
					<div class="form-group col-md-10 col-sm-9 country_num" style='padding-left: 0px; padding-right: 0px;'>
						<input type="text"
							style="border-bottom-left-radius: 0px; border-top-left-radius: 0px; border-left-style: none;"
							name="mnum"
							class="form-control mobile-number <?= (get_customer_data('phone_number') != '') ? 'active' : '' ?>"
			...
			<div class="text-center mt-4">
				...
				<?php if (!isset($_SESSION['customer_id'])): ?>
					...
					<?php if (isset($_SESSION['language_setting']) && $_SESSION['language_setting'] == 'vn') { ?>
						<p class="text-secondary mb-3">
							<?= $arrTranslate['By clicking Submit, you have read and agreed to our Terms and Conditions'] ?>.</p>
					<?php } else { ?>
						<p class="text-secondary mb-3">By clicking Submit, you have read and agreed to our <span
								class="font-bold text-primary btn-terms">Terms and Conditions</span>.</p>
					<?php } ?>
				...
				<button type="submit" class="btn btn-black"
					id="submitCustomer"><?= isset($_SESSION['customer_id']) ? $arrTranslate['Proceed'] : $arrTranslate['Submit'] ?></button>
...
<script>
	...
	$(document).ready(function () {
		...
		<?php if (isset($_GET['type']) && $_GET['type'] == 'log-in') { ?>
