...
function checkProfileID($profile_id){
	...
	return $profileIDF;
			...
			$email = 'guest@'.'sunniesstudios.com'.$_SESSION["store_code"].date('YmdHis');
			...
			$query="INSERT INTO profiles_info(
				...
				sales_person,
				address
			) VALUES (?,?,?,'guest', 'philippines', 'N/A', 'N/A', 'N/A', '1900-01-01',?,?,?,'630123456789',?,now(),?,'N/A'
			)";

			$stmt = mysqli_stmt_init($conn);

			if (mysqli_stmt_prepare($stmt, $query)) {
				mysqli_stmt_bind_param($stmt, 'ssssssss', $profileIDF, $_POST['lastname'], $_POST['firstname'], $_POST['age_range'], $_POST['gender'], $email, $_SESSION["store_code"], $_SESSION['id']);
				mysqli_stmt_execute($stmt);
			}
			...
			echo "	<script>	window.location='/sis/studios/".$_GET['path_loc']."/?page=select-store'</script>";
