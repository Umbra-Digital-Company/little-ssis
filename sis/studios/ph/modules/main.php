...
elseif(isset($_SESSION['user_type']) && $_SESSION['user_type'] == 3 ){
	echo '<script> window.location = "/sis/studios/vn/?page=store-home"; </script>';
}
else{
	echo '<script> window.location = "/"; </script>';
}

unset($_SESSION['language_setting']);
$page='';
$col='';
if (isset($_GET['page'])){
	$page = $_GET['page'];
	...
	<nav id="ssis_menu" class="mt-4">
		...
		<?php //if ( $_SESSION['userlvl'] == '2' ) : ?>
			...
			<ul class="nav-list nav flex-column">
				...
				<li>
					<a class="d-flex align-items-center" href="/sis/studios/func/logout.php?path_loc=ph">
...
<div id="admin-bar" class="d-flex align-items-center <?= ( $page == 'rate-us' ) ? 'justify-content-center' : 'justify-content-between' ?>">
	...
	<?php if ( isset($_SESSION['customer_page']) && $_SESSION['customer_page'] == 'YES' && !isset($_SESSION['doctor_progress']) ) : ?>
		...
		<?php if ( $page != 'rate-us' ) : ?>
			<!-- <div class="language">
				<img src="<?= get_url('images/icons') ?>/icon-language.png" alt="language" class="img-fluid">
			</div> -->
		<?php endif ?>
