$(document).ready(function () {
	...
	*/
	...
	$('#exit_customer_page').on('click', function() {
		$('.ssis-overlay').load("/sis/face/func/store/cancel-confirmation.php", function (d) {
			overlayContent(d);
			...
			let confirmAssistant = password => {
				if ( $('#password_confirmation').val() != password ) {
					...
					$.post('/sis/face/func/process/unset_login.php');
	...
	if ( $('#login_form').length ) {
		...
		$(".form-control").on('keyup', function(e) {
			if ( e.keyCode == 13 ) ajaxSubmit('/sis/face/func/loginConfirm.php', 'login_form');
		});

		$("#btnsubmit").on('click', function() {
			ajaxSubmit('/sis/face/func/loginConfirm.php', 'login_form');
		});
	...
	$('#switch_account').on('click', function (e) {
		e.preventDefault();
		$('.ssis-overlay').load("/sis/face/func/accounts.php", function (d) {
			overlayContent(d);
			...
			// submit login
			$("#btnsubmit").on('click', function () {
				ajaxSubmit('/sis/face/func/process/add_user2.php', 'add_account_login');
			});

			$(".form-control").on('keyup', function (e) {
				if (e.keyCode == 13) ajaxSubmit('/sis/face/func/process/add_user2.php', 'add_account_login');;
			});
	...
	$('#add_employee').on('click', function() {
		$('.ssis-overlay').load("/sis/face/func/account/create-account.php", function (d) {
			overlayContent(d);
		});
	});
	...
	const loadOverviewConfirmation = form => {
		...
		if (isChecked > 0) {
			$.post("/sis/face/func/process/overview_confirmation.php", $(form).serialize(), function (d) {
				overlayContent(d);
			});
		} else {
	...
	if ( $('.customer-profile-examine').length ) {
		...
		$('#send_to_cashier_overview').on('click', function() {
			...
			sendToCashier.on('click', function() {
				...
				$('.ssis-loading').fadeIn();
				$.ajax({
					url: '/sis/face/func/process/send_cashier.php',
					method: 'POST',
