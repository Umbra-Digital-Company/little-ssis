$(document).ready(function () {
	...
	*/
		...
		$('.ssis-overlay').load("/sis/studios/func/store/cancel-confirmation.php", function (d) {
			...
			let confirmAssistant = password => {
				if ( $('#password_confirmation').val() != password ) {
					...
					$.post('/sis/studios/func/process/unset_login.php');
	...
	if ( $('#login_form').length ) {
		...
		$(".form-control").on('keyup', function(e) {
			if ( e.keyCode == 13 ) ajaxSubmit('/sis/studios/func/loginConfirm.php', 'login_form');
		});

		$("#btnsubmit").on('click', function() {
			ajaxSubmit('/sis/studios/func/loginConfirm.php', 'login_form');
		});
	...
	$('#switch_account').on('click', function (e) {
		e.preventDefault();
		$('.ssis-overlay').load("/sis/studios/func/accounts.php", function (d) {
			overlayContent(d);
			...
			// submit login
			$("#btnsubmit").on('click', function () {
				ajaxSubmit('/sis/studios/func/process/add_user2.php', 'add_account_login');
			});

			$(".form-control").on('keyup', function (e) {
				if (e.keyCode == 13) ajaxSubmit('/sis/studios/func/process/add_user2.php', 'add_account_login');;
			});
	...
	$('#add_employee').on('click', function() {
		$('.ssis-overlay').load("/sis/studios/func/account/create-account.php", function (d) {
			overlayContent(d);
		});
	});
	...
	const loadOverviewConfirmation = form => {
		...
		if (isChecked > 0) {
			$.post("/sis/studios/func/process/overview_confirmation.php", $(form).serialize(), function (d) {
				overlayContent(d);
			});
		} else {
	...
	const useAccountLogin = () => {
		$.post("/sis/studios/func/store/store-sign-master.php", $("form#use_account").serialize(), function (d) {
			$('#msg').html("");
	...
	if ( $('.account-navigation').length ) {
		...
		$('form#create_account').submit(function (e) {
			...
			// validate gender
			if (r_gender.val() != '') {
				r_gender.next('label').css('border-color', '#a19f9d');
				v_gender = true;
			} else {
				r_gender.next('label').css('border-color', '#e54242');
				v_gender = false;
			}
	...
	if ( $('.customer-profile-examine').length ) {
		...
		$('#send_to_cashier_overview').on('click', function() {
			...
			sendToCashier.on('click', function() {
				...
				$('.ssis-loading').fadeIn();
				$.ajax({
					url: '/sis/studios/func/process/send_cashier.php',
					method: 'POST',
