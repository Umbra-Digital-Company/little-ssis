$(document).ready(function () {
	...
	*/
	...
	$('#exit_customer_page').on('click', function() {
		$('.ssis-overlay').load("/ssis/modules/store/assistant-confirmation.php", function (d) {
			overlayContent(d);
	...
	if ( $('#login_form').length ) {
		...
		$(".form-control").on('keyup', function(e) {
			if ( e.keyCode == 13 ) ajaxSubmit('/ssis/modules/loginsup.php', 'login_form');
		});

		$("#btnsubmit").on('click', function() {
			ajaxSubmit('/ssis/modules/loginsup.php', 'login_form');
		});
	...
	$('#switch_account').on('click', function (e) {
		e.preventDefault();
		$('.ssis-overlay').load("/ssis/modules/accounts.php", function (d) {
			overlayContent(d);
			...
			// submit login
			$("#btnsubmit").on('click', function () {
				ajaxSubmit('/ssis/modules/process/add_user2.php', 'add_account_login');
			});

			$(".form-control").on('keyup', function (e) {
				if (e.keyCode == 13) ajaxSubmit('/ssis/modules/process/add_user2.php', 'add_account_login');;
			});
	...
	$('#add_employee').on('click', function() {
		$('.ssis-overlay').load("/ssis/modules/account/create-account.php", function (d) {
			overlayContent(d);
		});
	});
	...
	const loadOverviewConfirmation = form => {
		...
		if (isChecked > 0) {
			$.post("/ssis/modules/process/overview_confirmation.php", $(form).serialize(), function (d) {
				overlayContent(d);
			});
		} else {
	...
	const useAccountLogin = () => {
		$.post("modules/store/store-sign-master.php", $("form#use_account").serialize(), function (d) {
			$('#msg').html("");
	...
	if ( $('.account-navigation').length ) {
		...
		$('form#create_account').submit(function (e) {
			...
			if (r_country.val() != 'philippines') {
				...
				if (v_lname == true && v_fname == true && v_mname == true && v_address == true && v_country == true && v_province == true && v_city == true && v_brgy == true && v_email == true && v_mobile == true && v_bdate == true && v_age == true && v_gender == true) {
	...
	if ( $('.customer-profile-examine').length ) {
		...
		$('#send_to_cashier_overview').on('click', function() {
			...
			sendToCashier.on('click', function() {
				...
				$('.ssis-loading').fadeIn();
				$.ajax({
					url: './modules/process/send_cashier.php',
					method: 'POST',
