...
$querypn .=' FROM 
                poll_51_face_new p
            WHERE p.data_cgc IN("DCGC0017","DCGC0020","DCGC0024","DCGC0004","DCGC0019", "DCGC0076")
                    '.$priceWhere.'
            ORDER BY 
                p.item_name ASC;';
...
$arrColorData = [];
    $queryAll = 'SELECT DISTINCT
                    ...
                    WHERE 
                        p.data_cgc IN("DCGC0017","DCGC0020","DCGC0024","DCGC0004","DCGC0019", "DCGC0076")
...
if(!isset($_GET['filter'])){
    ...
    $queryAll = "SELECT
                    ...
                    p51.item_code
                     ...
                     WHERE ss.category = 'cheeks' AND active = 1
...
for ($i=0; $i < sizeOf($arrProduct); $i++) { 
    ...
    $arrProductsSorted[$curStyle]["colors"][$curSKU]["data_cgc"]     = $arrProduct[$i]["data_cgc"];
    $arrProductsSorted[$curStyle]["colors"][$curSKU]["sub_category"]     = $arrProduct[$i]["sub_category"];
    $arrProductsSorted[$curStyle]["colors"][$curSKU]["category"]     = $arrProduct[$i]["category"];
