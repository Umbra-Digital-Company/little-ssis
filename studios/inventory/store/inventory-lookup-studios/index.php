...
if(isset($_GET['frame_code'])){
	for ($i=0;$i<sizeof($arrStudiosStore);$i++) {
			$arrStoreData[] = ['storelab' => 'store', 'store_id' => $arrStudiosStore[$i]['store_id'],'store_name' => $arrStudiosStore[$i]['store_name'],'phone_number' => $arrStudiosStore[$i]['phone_number']];
	}
...
$(document).ready(function() {
	...
	function getData($rowCount){
		$.get('get_data.php',{frame_code:frame, arrStore: arrStore[$rowCount] },function(result){
				$('table tbody').append(result);
