...
$filter_page = 'history_store_studios';
				...
				<form method="GET" class="filter-body">
							...
							<div class="d-flex align-items-center">
								...
								<select name="filterSender" id="filterSender" class="form-control select2" <?= ( (isset($_GET['filterProcess']) && $_GET['filterProcess']!='in') || !isset($_GET['filterProcess'])) ? 'disabled="disabled"' : '' ?>>
									...
									<optgroup label="STORE NAME">
										<?php for ($i=0;$i<sizeof($arrStudiosStore);$i++) { ?>
											<option value="<?= $arrStudiosStore[$i]['store_id'] ?>" <?= (isset($_GET['filterSender']) && $_GET['filterSender']==$arrStudiosStore[$i]['store_id']) ? 'selected="selected"' : ''; ?>><?= ucwords(str_replace(['ali','mw','sm','-'], ['ALI','MW','SM',' '], strtolower($arrStudiosStore[$i]['store_name']))) ?></option>
										<?php } ?>
									</optgroup>
						...
						<div class="col-12 col-md-6">
							<div class="d-flex align-items-center">
								...
								<select name="filterReceiver" id="filterReceiver" class="form-control select2" <?= ( (isset($_GET['filterProcess']) && $_GET['filterProcess']!='out') || !isset($_GET['filterProcess'])) ? 'disabled="disabled"' : '' ?>>
									...
									<optgroup label="STORE NAME">
										<?php for ($i=0;$i<sizeof($arrStudiosStore);$i++) { ?>
											<option value="<?= $arrStudiosStore[$i]['store_id'] ?>" <?= (isset($_GET['filterReceiver']) && $_GET['filterReceiver']==$arrStudiosStore[$i]['store_id']) ? 'selected="selected"' : ''; ?>><?= ucwords(str_replace(['ali','mw','sm','-'], ['ALI','MW','SM',' '], strtolower($arrStudiosStore[$i]['store_name']))) ?></option>
										<?php } ?>
									</optgroup>
					...
					<div class="text-center mt-5 pb-5">
						...
						<?php if(isset($_GET['date'])) { ?>
							...
							<a href="/studios/inventory/store/studios-history/" class="ml-0 mt-3 mt-sm-0 mt-sm-3 d-block d-sm-inline ml-sm-3">
			...
			<div id="inventory-receive" class="mt-4">
				...
				<?php } else { ?>
					...
					<div class="mt-5 text-center">
						<a href="/studios/inventory/store/studios-history/" class="text-uppercase text-danger">clear search</a>
					</div>
