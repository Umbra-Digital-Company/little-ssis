...
$page = 'audit';
$page_url = 'studios-physical-count';

$filter_page = 'phyiscal_count_audit_studios';
			...
			<div class="row" style="max-width: 100%;">
 				...
 				<div class="col-12 col-lg-8" style="overflow:hidden">
					...
					<div id="excel-inventory" class="custom-card p-0">
						<form action="/inventory/process/studios_actual_count.php" method="POST" autocomplete="off" class="table-default table-responsive" style="max-width: 100%;">
								...
								<select name="filterStores" id="id" class="select2 form-control" required>
									...
									<optgroup label="STORE NAME">
										<?php for ($i=0;$i<sizeof($arrStudiosStore);$i++) { ?>
											<option value="<?= $arrStudiosStore[$i]['store_id'] ?>" <?= isset($_GET['filterStores']) && $_GET['filterStores']==$arrStudiosStore[$i]['store_id'] ? 'selected' : '' ?>><?= ucwords(str_replace(['ali','sm','mw'],['ALI','SM','MW'],strtolower($arrStudiosStore[$i]['store_name']))) ?></option>
										<?php } ?>
									</optgroup>
