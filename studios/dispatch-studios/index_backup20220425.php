...
require $sDocRoot."/includes/v2/navbar_studios.php";
require $sDocRoot."/includes/v2/functions.php";
require $sDocRoot."/includes/grab_customer_list_dispatch_studios.php";
// require $sDocRoot."/includes/grab_laboratory.php";
require $sDocRoot."/includes/grab_stores_studios.php";
...
for ($i=0; $i < sizeOf($arrStoresStudios); $i++) { 
	...
	if($arrStoresStudios[$i]['store_id'] == $_SESSION['user_login']['store_code'] ) {

		$storeName = $arrStoresStudios[$i]['store_name'];
			...
			<section class="ssis-section">
	            ...
	            <div class="section-outter closed">				
					...
					<?php if($_SESSION['store_code'] == '147' || $_SESSION['store_code'] == '148'  || $_SESSION['store_code'] == '149') { ?>
								 ...
								 <!-- <button  class="btn-payo-refresh">Refresh Payo</button>  -->
						...
						<div class="row no-gutters align-items-center justify-content-between">									
							...
							<div class="col text-center">
								<select class="filter-select" id="filterStore">
									...
									<!-- <option value="all" <?php if($_GET['store'] == 'all') { echo "selected"; } ?>>All</option> -->
									
									<?php 

										for($i=0; $i<sizeof($arrStoresStudios); $i++){

											$storeName = str_replace("Sm ", "SM ", ucwords(str_replace('-',' ',$arrStoresStudios[$i]["store_name"])));
											$storeID = $arrStoresStudios[$i]['store_id'];
											...
											echo '<option value="'.$storeID.'"'.$sel.'>STUDIOS '.$storeName.'</option>';
		        	...
		        	<div class="section-inner">	
							...
							<div id="chat-room">
								<div class="chat-head text-center text-uppercase">
									<div class="row no-gutters align-items-center justify-content-between">
										<p class="m-0 text-uppercase font-weight-bold">chats</p>
										<span id="hide-chat-room"><img src="/assets/images/icons/danger-close-icon.png" alt="close" class="img-fluid"></span>
									</div>
								</div>
								<div class="chat-list">
									<?php for ($ml = 0; $ml<sizeof($arrMessageLatest); $ml++) {
										
										$account = 'acct_id_'.$ml; # account id for onClick open modal
										$person = 'person'; # name of the sender
										$location = 'branch'; # branch/location of the sender

										?>
								<a href="?search=<?= $arrMessageLatest[$ml]["po_number"] ?>&seen=<?= $arrMessageLatest[$ml]["order_po_id"] ?>">
										<div class="chat-item" data-id="<?= $account ?>" data-toggle="modal" data-target="#messengerModal">
											<div class="row no-gutters align-items-center">
											
												<div class="name">
													<p class="mb-1 font-weight-bold"><?= ucwords($arrMessageLatest[$ml]["po_number"]) ?></p>
													
												</div>
											</div>
											<p class="text-secondary m-0"><?= ucwords($arrMessageLatest[$ml]["message"]) ?></p>
										</div>
										</a>
									<?php } ?>
								</div>
								<div class="chat-footer">
										<input type="text" class="form-control" id="search-chat-room" autocomplete="off" placeholder="Search person">
										<span id="clear-search-chat"><img src="/assets/images/icons/danger-close-icon.png" alt="close" class="img-fluid"></span>
								</div>
							</div>
		        		...
		        		<div class="row align-items-top justify-content-between no-gutters mt-80">
							...
							<div>
								...
								<?php if($_SESSION['store_code'] != '150' &&  $_SESSION['store_code'] != '142'){ ?>
									<label class="btn btn-info ml-2" id="btnPackage" data-value="package" style="border-radius: 10px;">PACKAGE LIST</label>
								<?php } ?>
											...
											<table cellpadding="0" cellspacing="0">
												<thead>
													<tr class="row100 head">
														...
														<th class="cell100 small column3" nowrap>Processed</th>
														<th class="cell100 small column3" nowrap>Production</th>
														<th class="cell100 small column3" nowrap>completed</th>
														<th class="cell100 small column3" nowrap>Received</th>
														<!-- <th class="cell100 small column3" nowrap>Packaging</th> -->
														<th class="cell100 small column3" nowrap>Dispatched</th>
												...
												<tbody>
													...
													<tr class="row100 body">
														...
														?>">
															...
															<?php 
																if($arrCustomer[$i]['lens_option'] == 'without prescription' && $arrCustomer[$i]['product_code'] == 'M100'){
																	echo $arrCustomer[$i]['product_upgrade'];

																}elseif($arrCustomer[$i]["status"]=='return'){
																	echo 'Returned';
																}
																elseif($_SESSION['store_code']!=$arrCustomer[$i]["store_id"]){
																...
																elseif($arrCustomer[$i]["product_upgrade"]=='sunnies_studios'){
																	echo $arrCustomer[$i]["item_description_studios"];
																}
                                                                elseif($arrCustomer[$i]["lens_option"]=='without prescription' &&  $arrCustomer[$i]['product_upgrade']!='kids_screen_safe'){

																	if($arrCustomer[$i]["product_upgrade"]=='fashion_lens'){
																		echo $arrCustomer[$i]["item_description_specs"];
																	}elseif($arrCustomer[$i]["product_code"]=='M100'){
																		echo "Merch";
																	}elseif($arrCustomer[$i]["product_upgrade"]=='G100'){
																		echo "ANTI-RAD";
																...
																elseif($order_status=='returned' || $order_status=='return'){
																	...
																	if($production == 'y' && $lab_status=='n'){
																		...
																		if(date("Y-m-d h:i:sa")>=$arrCustomer[$i]['target_date']){
															...
															<span class="small">
																...
																<?php 
																	...
																	if($arrCustomer[$i]['lab_production_date']=='1910-01-01 22:59:52' || $arrCustomer[$i]['lab_production_date']=='0000-00-00 00:00:00') { 
																	} 
																	else { 

																		echo $arrCustomer[$i]['lab_production_date']; 
												...
												elseif($arrCustomer[$i]["status"]=='return'){
															...
															<span class="small">
																...
																<?php 
																	...
																	if($arrCustomer[$i]['lab_status_date']=='1910-01-01 22:59:52' || $arrCustomer[$i]['lab_status_date']=='0000-00-00 00:00:00') { 
																	} 
																	else { 

																		echo $arrCustomer[$i]['lab_status_date']; 
														...
														">
															...
															<?php
																...
																if ( $arrCustomer[$i]["status"]=='return') {
																	...
																	echo "Returned <br><span class='small'>".$arrCustomer[$i]['received_stat_date']."</span>";
																...
																elseif ( ($receive=='n' && $lab_status == 'y') || ($arrCustomer[$i]['product_upgrade']=='special_order' && $receive!='y') || ($arrCustomer[$i]['product_upgrade']=='kids_screen_safe' && $receive!='y')  ) { 

															?>

															<input class="sr-only checkbox-original-style" id="checkbox_<?php echo $arrCustomer[$i]["id"] ?>" type="checkbox" name="receive[<?php echo $arrCustomer[$i]["id"] ?>]" value="y">
															<label class="checkbox-custom-style" for="checkbox_<?php echo $arrCustomer[$i]["id"] ?>"></label>
															...
															<br>
															<span class="small"><?php  if($arrCustomer[$i]['received_stat_date']=='1910-01-01 22:59:52' || $arrCustomer[$i]['received_stat_date']=='0000-00-00 00:00:00') { } else { echo $arrCustomer[$i]['received_stat_date']; } ?></span>
														...
														<?php
															...
															// 	$dis =  'disabled';
															// }
															// elseif($arrCustomer[$i]['lens_code'] =='L035' && $arrCustomer[$i]['lens_option'] == 'without prescription' && ($receive == '' || $receive == 'n') ){
															// 	$dis =  'disabled';
															// }
															// elseif($arrCustomer[$i]['lens_option'] == 'with prescription'){
															// 	if( ($receive == '' || $receive == 'n') && $packaging == ''){
															// 		$dis =  'disabled';
															// 	}
															// }

															// if($dis == 'disabled' && ($arrCustomer[$i]['lens_option'] == 'without prescription' || $arrCustomer[$i]['product_upgrade'] == 'sunnies_studios' || $arrCustomer[$i]['product_code'] == 'M100')){
															// 	$bg_color = 'bg-info';
															// }elseif($dis ==  'disabled' && ($arrCustomer[$i]['lens_option'] == 'with prescription' || $arrCustomer[$i]['lens_code'] =='SO1001' || $arrCustomer[$i]['lens_option'] =='kids plano screen safe' )){
															// 	$bg_color = 'bg-success';
															// }
														?>
														<!-- <td nowrap class="cell100 small column7 text-center <?= $bg_color ?>" >
															<?php
																if($dis == 'disabled'){
																	echo 'Yes';
																}
															...
															?>
																<button type="button" class="btn btn-success  create-packaging center-block" order-id="<?= $arrCustomer[$i]['order_id']?>" po-number="<?= $arrCustomer[$i]['po_number']?>" customer-name="<?= ucwords( $arrCustomer[$i]['first_name'] )." ".ucwords( $arrCustomer[$i]['last_name'] ) ?>" orders-specs-id="<?= $arrCustomer[$i]['orders_specs_id'] ?>" >Package</button>
														...
														<?php 
															if ( $dispatch == 'y' && $arrCustomer[$i]['status'] != 'returned' ) { 

																echo 'bg-success'; 

                                                            } 
                                                            elseif($arrCustomer[$i]["lens_option"]=='without prescription' &&  $arrCustomer[$i]['product_upgrade']!='kids_screen_safe'){
                                                            	if($arrCustomer[$i]["product_upgrade"]!='fashion_lens' && $arrCustomer[$i]["product_code"]!='M100'){
                                                            		echo 'bg-info';
																}
                                                            }
															elseif($_SESSION['store_code']!=$arrCustomer[$i]["store_id"]){
															...
															elseif ( $dispatch == 'y' && $arrCustomer[$i]['status'] == 'returned' || $receive == 'r' ) { 
														...
														?>">
															...
															<?php 
																...
																}elseif($arrCustomer[$i]["lens_option"]=='without prescription' &&  $arrCustomer[$i]['product_upgrade']!='kids_screen_safe' && $arrCustomer[$i]["packaging"]!='y'){

																	if(($arrCustomer[$i]["product_upgrade"]=='fashion_lens' || $arrCustomer[$i]["product_code"]=='M100' || preg_match("/AR/i",$arrCustomer[$i]["product_code"]) ) &&  $dispatch == 'n'){
																	?>
																		<button <?= ($receive== 'n' && $arrCustomer[$i]["product_code"]!='M100' && $arrCustomer[$i]["product_upgrade"]!='fashion_lens' && $arrCustomer[$i]["product_upgrade"]!='sunnies_studios'  && $arrCustomer[$i]["product_upgrade"]!='G100'   ) ? 'disabled="disabled' : ''; ?> type="button" class="btn btn-secondary create-signature center-block" id="create-signature_<?php echo $arrCustomer[$i]['id']; ?>" product-code="<?= $arrCustomer[$i]['product_code']?>" product-upgrade="<?= $arrCustomer[$i]['product_upgrade']?>" order-id="<?= $arrCustomer[$i]['order_id']?>" po-number="<?= $arrCustomer[$i]['po_number']?>" payment-date="<?=date('Y-m-d', strtotime($arrCustomer[$i]['payment_date'])) ?>"  customer-name="<?= ucwords( $arrCustomer[$i]['first_name'] )." ".ucwords( $arrCustomer[$i]['last_name'] ) ?>" orders-specs-id="<?= $arrCustomer[$i]['orders_specs_id'] ?>">dispatch</button>
																	<?php
																	}else{
																	echo 'N/A';
																	}
																...
																elseif (  $dispatch == 'n' &&  $receive=='y'  && $arrCustomer[$i]["packaging"]!='y' ) {

															?>

															<button <?= ($receive== 'n'   ) ? 'disabled="disabled' : ''; ?> type="button" class="btn btn-secondary create-signature center-block" id="create-signature_<?php echo $arrCustomer[$i]['id']; ?>" product-code="<?= $arrCustomer[$i]['product_code']?>" product-upgrade="<?= $arrCustomer[$i]['product_upgrade']?>"  order-id="<?= $arrCustomer[$i]['order_id']?>" po-number="<?= $arrCustomer[$i]['po_number']?>" payment-date="<?=date('Y-m-d', strtotime($arrCustomer[$i]['payment_date'])) ?>" customer-name="<?= ucwords( $arrCustomer[$i]['first_name'] )." ".ucwords( $arrCustomer[$i]['last_name'] ) ?>" orders-specs-id="<?= $arrCustomer[$i]['orders_specs_id'] ?>">dispatch</button>

															<?php 

																} 
																elseif( $arrCustomer[$i]['status'] == 'returned'  || $receive=='r') { 
															...
															<?php 
																...
																}elseif($dispatch == 'y' || $arrCustomer[$i]["packaging"]=='y'){
															...
															<?php 
																	echo $arrCustomer[$i]['store_dispatch_date'];
																} 
													...
													<?php if($_SESSION['store_code'] == '147' ||  $_SESSION['store_code'] == '148'  || $_SESSION['store_code'] == '149'  )  { ////////////////// PROCESSING COLUMN 
														
														if($arrCustomer[$i]['courier']!=''){?>

															<td style="border-right: 2px solid #000000;" nowrap class="cell100 small">
															<button type="button" class="btn btn-success center-block sendpayo" id="sendtoCourier_<?php echo $arrCustomer[$i]['orders_specs_id']; ?>" data-order-id="<?= $arrCustomer[$i]['order_id']; ?>" data-po-number="<?= $arrCustomer[$i]['po_number']; ?>" > Resend to Payo</button>	
																</td>
																
																<td style="border-right: 2px solid #000000;" nowrap class="cell100 small">
																<button class="btn btn-primary refresh_single_payo" id="refreshPayo_<?= $arrCustomer[$i]["orders_specs_id"] ?>">REFRESH Order PAYO</button>
																</td>
												...
												<?php 		}else{ ?>
													...
													<td style="border-right: 2px solid #000000;" nowrap class="cell100 small">
													<?php if($receive=='y'){ ?>
														<button type="button" class="btn btn-success center-block sendpayo" id="sendtoCourier_<?php echo $arrCustomer[$i]['orders_specs_id']; ?>" data-order-id="<?= $arrCustomer[$i]['order_id']; ?>" data-po-number="<?= $arrCustomer[$i]['po_number']; ?>" > Send to Payo</button>
														<?php } ?>
													</td>
												...
												<?php } ?>
													...
													<?php if($receive=='y'){ ?>
														<button type="button" class="btn btn-success center-block printpayo" id="PrintWayBill_<?php echo $arrCustomer[$i]['orders_specs_id']; ?>" data-order-id="<?= $arrCustomer[$i]['order_id']; ?>" data-po-number="<?= $arrCustomer[$i]['po_number']; ?>" data-invoice-no="<?= $arrCustomer[$i]['courier_no']; ?>" > Print WayBill</button>
														<?php } ?>
													</td>
													
													<td style="border-right: 2px solid #000000;" nowrap class="cell100 small">
													<?php 	if($arrCustomer[$i]['courier']!=''){
													
													
													if($arrCustomer[$i]['courier']=='Payo'){ ?>
																Courier : Payo x <?= $arrCustomer[$i]['sub_courier'];?> <br>
																  Status: <?= $arrCustomer[$i]['payo_status'];?> <br>
																 Tracking No. : <?= $arrCustomer[$i]['tracking_no']; ?><Br>
																 Invoice No. : <?= $arrCustomer[$i]['courier_no'];?>

																<?php  
																}else{
																			echo $arrCustomer[$i]['courier_no']; 
																}?>
													<?php  } ?>
													</td>
														<?php
														...
														<?php if($_SESSION['store_code']!=$arrCustomer[$i]["store_id"]){
															...
															<button type="button" class="btn btn-warning btn-remake-order center-block" id="remake_<?php echo $arrCustomer[$i]['id']; ?>" data-customer-id="<?= $arrCustomer[$i]['id']; ?>" data-order-specs-id="<?= $arrCustomer[$i]['orders_specs_id']; ?>" data-order-id="<?= $arrCustomer[$i]['order_id']; ?>" data-po-number="<?= $arrCustomer[$i]['po_number']; ?>" data-customer-name="<?= ucwords( $arrCustomer[$i]['first_name'] )." ".ucwords( $arrCustomer[$i]['last_name'] ); ?>" style="color: #ffffff;">remake</button>
														<?php } ?>
														</td>
														<td style="border-right: 2px solid #000000;">
														<?php if($_SESSION['store_code']!=$arrCustomer[$i]["store_id"]){
...
// alert(orderidspecs);
				...
				$("#btnSearch").click(function() {
					...
					filterURL = "/dispatch-studios/?search=" + filterA + "&page=1";
				...
				$('#filterPage').on('change', function() {
					...
					filterURL = "/dispatch-studios/?search=" + filterA + "&page=" + filterB;
					filterURL += '<?= (isset($_GET["store"])) ? "&store=".$_GET["store"] : "" ?>';
					window.location.href = filterURL;
...
toggleFilter('hide');
				...
				$(document).ready(function(){					
					...
					// 		$("#overlay_"+id).load('../process/dispatch/dispatch_detail_v2.php?&cn=' + id+'&name='+customer+'&po_number='+po_number+'&order_id='+order_id+'&orders_specs_id='+orders_specs_id+'&product_code='+product_code+'&product_upgrade='+product_upgrade+'&payment_date='+payment_date);
				 ...
				 //    remakeOrder();
					...
					$('#cancelThisOrder').click(function() {
							...
							if(thisCancelReason=='' ){
								...
								$.ajax({
									...
									type: "POST",
									url: "../process/dispatch_studios/cancel.php",
					...
					$('#filterStore').on('change', function() {
						...
						filterURL = "/dispatch-studios/?store=" + filterStore;
