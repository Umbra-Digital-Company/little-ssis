...
function grabItemsSoldSpecs() {
	...
	$arrBestFrame = array();
	 ...
	 $query = 	"SELECT  
				    ...
				    COUNT(
				    	case when os.lens_option = 'with prescription' AND os.status NOT IN ('cancelled','return','returned' ) and  os.prescription_vision != 'non-prescription' and  os.prescription_vision != 'non_prescription'
						 then 1 end
				    ) AS 'count_with_prescription',
				    COUNT(
				    	case when os.product_upgrade = 'fashion_lens' AND os.status = 'paid' AND os.status NOT IN ('cancelled','return','returned' ) then 1 end
				    ) AS 'count_without_prescription',
				    COUNT(
				    	case when os.lens_option = 'lens only' AND os.status NOT IN ('cancelled','return','returned' )  then 1 end
				    ) AS 'count_lens_only',
				    COUNT(
				    	case when os.prescription_vision = 'single_vision_stock' AND os.status NOT IN ('cancelled','return','returned' )  then 1 end
				    ) AS 'count_single_vision_stock',
				    COUNT(
				    	case when os.prescription_vision = 'single_vision_rx' AND os.status NOT IN ('cancelled','return','returned' )  then 1 end
				    ) AS 'count_single_vision_rx',
				    COUNT(
				    	case when os.prescription_vision = 'double_vision_stock' AND os.status NOT IN ('cancelled','return','returned' )  then 1 end
				    ) AS 'count_double_vision_stock',
				    COUNT(
				    	case when os.prescription_vision = 'double_vision_rx' AND os.status NOT IN ('cancelled','return','returned' )  then 1 end
				    ) AS 'count_double_vision_rx',
				    COUNT(
				    	case when os.prescription_vision = 'progressive_stock' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						when os.prescription_vision = 'progressive_conventional' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						 end
				    ) AS 'count_progressive_stock',
				    COUNT(
				    	case when os.prescription_vision = 'progressive_rx' AND os.status NOT IN ('cancelled','return','returned' )  then 1 
						when os.prescription_vision = 'progressive_conventional_rx' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						end
				    ) AS 'count_progressive_rx',
				    COUNT(
				    	case when os.product_upgrade = 'special_order' AND os.status NOT IN ('cancelled','return','returned' ) then 1 end
				    ) AS 'count_special_order',
				    COUNT(
				    	case when os.prescription_vision = '' then 1 end
				    ) AS 'count_none',
				    COUNT(
				    	case when os.prescription_vision = 'single_vision_standard'  AND os.status NOT IN ('cancelled','return','returned' ) then 1
						when os.prescription_vision = 'standard_single_vision' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						when os.prescription_vision = 'standard_extended_plus' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						when os.prescription_vision = 'standard_extended_minus_1' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						when os.prescription_vision = 'standard_extended_minus_2' AND os.status NOT IN ('cancelled','return','returned' )   then 1
						when os.prescription_vision = 'single_vision_extended' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						when os.prescription_vision = 'single_vision_customized' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						when os.prescription_vision = 'customized_extended_plus'  AND os.status NOT IN ('cancelled','return','returned' ) then 1
						when os.prescription_vision = 'customized_extended_minus_1' AND os.status NOT IN ('cancelled','return','returned' )  then 1
						 end
				    ) AS 'single_vision_standard',
				    COUNT(
				    	case when os.prescription_vision = 'non-prescription'  AND os.status NOT IN ('cancelled','return','returned' )then 1 
						when os.prescription_vision = 'non_prescription' AND os.status NOT IN ('cancelled','return','returned' ) then 1 
						end
				    ) AS 'non-prescription',  
	...
	if (mysqli_stmt_prepare($stmt, $query)) {
		...
		mysqli_stmt_bind_result($stmt, $result1, $result2, $result3, $result4, $result5, $result6, $result7, $result8, $result9, $result10, $result11,
		 $result12, $result13, $result14, $result15, $result16, $result17, $result18, $result19, $result20, $result21, $result22, $result23, $result24, $result25, $result26, $result27, $result28, $result29, $result30);
	...
	$query = 	"SELECT
				...
				FROM
					orders o
						...
						LEFT JOIN profiles_info pi
							ON pi.profile_id = os.profile_id
							LEFT JOIN  orders od  ON od.profile_id!=o.profile_id AND od.email_address=o.email_address 
				WHERE
					".$qDate."
					".$specStore."
					".$setInternational."
