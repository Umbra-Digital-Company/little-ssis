...
require $sDocRoot."/includes/dashboard_supervisor_v2/functions.php";
...
$arrReasonsFrameOnly = array(
	...
	"fashion use  (no grade)f" => array(

		"reason" => "Fashion use  (no grade)",
		"count" => 0

	),
	"cannot wait (rush order)" => array(
...
for ($i=0; $i < sizeOf($arrPrescriptions); $i++) { 
	...
	if(!isset($arrTopLenses[$curLensCode]["count"])){
		$arrTopLenses[$curLensCode]["count"] = 1;
	}else{
...
for ($i=0; $i < sizeOf($arrPrescriptions); $i++) { 
	...
	$curVision = $arrPrescriptions[$i]['prescription_vision'];
	if(isset($arrPRXTypes[$curVision]['lenses'])){
...
echo '<div class="loader" id="data-top-blocks-prx-types" data-sv-stock-count="'.( $arrPRXTypes['single_vision_stock']['count'] ).'" data-sv-rx-count="'.( $arrPRXTypes['single_vision_rx']['count'] ).'"
 data-dv-stock-count="'.( $arrPRXTypes['double_vision_stock']['count'] ).'" data-dv-rx-count="'.( $arrPRXTypes['double_vision_rx']['count'] ).'"
...
for ($i=0; $i < sizeOf($arrItemsMerch); $i++) { 
	...
	// Set current data
	if(isset($arrItemsMerch['item_name']) && $arrItemsMerch['item_name'] != NULL && $arrItemsMerch['item_name'] != '') {
			$curItem = $arrItemsMerch['item_name'];
	}
	else {
		if(isset($arrItemsMerch['item_name_sunnies'])){
...
for ($i=0; $i < sizeOf($arrFrameData); $i++) { 
	...
	if($arrFrameData[$i]['collection_code'] != NULL) {
		...
		if(!isset($arrFrameStats['collections'][$arrFrameData[$i]['collection_code']]['count'])){
			$arrFrameStats['collections'][$arrFrameData[$i]['collection_code']]['count'] = 1;
		}else{
	...
	if($arrFrameData[$i]['group_category_code'] != NULL) {
		...
		if(!isset($arrFrameStats['group_categories'][$arrFrameData[$i]['group_category_code']]['count'])){
			$arrFrameStats['group_categories'][$arrFrameData[$i]['group_category_code']]['count'] = 1;
		}else{
	...
	if($arrFrameData[$i]['finish_code'] != NULL) {
		...
		if(!isset($arrFrameStats['finish'][$arrFrameData[$i]['finish_code']]['count'])){
			$arrFrameStats['finish'][$arrFrameData[$i]['finish_code']]['count'] = 1;
		}else{
	...
	if($arrFrameData[$i]['general_color_code'] != NULL) {
		...
		if(!isset($arrFrameStats['general_colors'][$arrFrameData[$i]['general_color_code']]['count'])){
			$arrFrameStats['general_colors'][$arrFrameData[$i]['general_color_code']]['count'] = 1;
		}else{
	...
	if($arrFrameData[$i]['material_code'] != NULL) {
		...
		if(!isset($arrFrameStats['materials'][$arrFrameData[$i]['material_code']]['count'])){
			$arrFrameStats['materials'][$arrFrameData[$i]['material_code']]['count'] = 1;
		}else{
	...
	if($arrFrameData[$i]['product_seasonality_code'] != NULL) {
		...
		if(!isset($arrFrameStats['product_seasonality'][$arrFrameData[$i]['product_seasonality_code']]['count'])){
			$arrFrameStats['product_seasonality'][$arrFrameData[$i]['product_seasonality_code']]['count'] = 1;
		}else{
	...
	if($arrFrameData[$i]['shape_code'] != NULL) {
		...
		if(!isset($arrFrameStats['shapes'][$arrFrameData[$i]['shape_code']]['count'])){
			$arrFrameStats['shapes'][$arrFrameData[$i]['shape_code']]['count'] = 1;
		}else{
	...
	if($arrFrameData[$i]['size_code'] != NULL) {
		...
		if(!isset($arrFrameStats['sizes'][$arrFrameData[$i]['size_code']]['count'])){
			$arrFrameStats['sizes'][$arrFrameData[$i]['size_code']]['count'] = 1;
		}else{
...
// Grab all the AntiRad orders
$arrItemsAntiRad = grabItemsSoldAntiRad();

/////////////// count frames

$totalAntiRadCount = 0;

for ($i=0; $i < sizeOf($arrItemsAntiRad); $i++) { 

	// Set current data
	if(isset($arrItemsAntiRad[$i]['item_name']) && $arrItemsAntiRad[$i]['item_name'] != NULL && $arrItemsAntiRad[$i]['item_name'] != '') {
			$curItem = $arrItemsAntiRad[$i]['item_name'];
	}
	else {
		if(isset($arrItemsAntiRad[$i]['item_name_sunnies'])){
			$curItem = $arrItemsAntiRad[$i]['item_name_sunnies'];
		}

	};	

	$totalAntiRadCount += $arrItemsAntiRad[$i]['count_antirad'];
	
};

/////////////// top selling frame

if(!empty($arrItemsAntiRad)) {

	$bestAntiRadItem = ucwords(strtolower($arrItemsAntiRad[0]['item_name']));
...
echo '<div class="loader" id="data-top-blocks-antirad" data-antirad-items-sold="'.( number_format($totalAntiRadCount, 0, '.', ',') ).'" data-antirad-items-sold-raw="'.$totalAntiRadCount.'" data-best-antirad-item="'.$bestAntiRadItem.'"></div>';
