...
if(isset($_GET['date']) && $_GET['date'] != 'custom') {
	...
	switch ($_GET['date']) {
		...
		case 'week':
			$w = date('N');
			$week_start = date('F d, Y', strtotime('-'.($w-1).' days'));
			$week_end = date('F d, Y', strtotime('+'.(7-$w).' days'));
		...
		case 'month':
			...
			break;
		case 'pmonth':
			$date_title = date('F, Y', strtotime("-1 months"));
			break;
		case 'monthYear':
			$dateMonthYear = $_GET['data_range_year']."-".(sprintf("%02d", $_GET['data_range_month'])."-01");
			$date_title = date('F, Y',strtotime($dateMonthYear));
			break;
		case 'year':
			$date_title = date('Y');
			break;
