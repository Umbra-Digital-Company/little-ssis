...
if((isset($_GET["date"]) && $_GET['date'] != "custom") || $customMonth) {
	...
	// Date Filter on Branch Link
	$date_filter = "date=".$_GET['date'];
...
elseif(isset($_GET['data_range_start_month'])) {
	...
	// Date Filter on Branch Link
	$date_filter = "date=custom&data_range_start_month=".$_GET['data_range_start_month']."&data_range_start_day=".$_GET['data_range_start_day']."&data_range_start_year=".$_GET['data_range_start_year']."&data_range_end_month=".$_GET['data_range_end_month']."&data_range_end_day=".$_GET['data_range_end_day']."&data_range_end_year=".$_GET['data_range_end_year']."";
...
$removeStoreIDs = "AND o.store_id NOT IN ('142', '1000', '900', '126', '130', '787', '788','150', '999','139','155')";
$removeStoreLocations = "sl.store_id NOT IN ('142', '1000', '900', '126', '130', '787', '788','150', '999','139','155')";
...
function grabItemsSoldSpecs() {
	...
	$arrBestFrame = array();
	 $query = 	"SELECT  
				    ...
				    COUNT(
				    	case when os.lens_option = 'lens only'  AND os.payment = 'y' And os.status NOT IN ('return','cancelled','returned','failed','for exam', 'for payment', '' ) then 1 end
				    ) AS 'count_lens_only',
				    COUNT(
				    	case when os.prescription_vision = 'single_vision_stock' AND os.payment = 'y' And os.status NOT IN ('return','cancelled','returned','failed','for exam', 'for payment', '' ) then 1 end
				    ) AS 'count_single_vision_stock',
				    COUNT(
				    	case when os.prescription_vision = 'single_vision_rx' AND os.payment = 'y' And os.status NOT IN ('return','cancelled','returned','failed','for exam', 'for payment', '' ) then 1 end
				    ) AS 'count_single_vision_rx',
				    COUNT(
				    	case when os.prescription_vision = 'double_vision_stock'  AND os.payment = 'y' And os.status NOT IN ('return','cancelled','returned','failed','for exam', 'for payment', '' ) then 1 end
				    ) AS 'count_double_vision_stock',
				    COUNT(
				    	case when os.prescription_vision = 'double_vision_rx' AND os.payment = 'y' And os.status NOT IN ('return','cancelled','returned','failed','for exam', 'for payment', '' ) then 1 end
				    ) AS 'count_double_vision_rx',
				    COUNT(
				    	case when os.prescription_vision = 'progressive_stock'  AND os.payment = 'y' And os.status NOT IN ('return','cancelled','returned','failed','for exam', 'for payment', '' ) then 1 end
				    ) AS 'count_progressive_stock',
				    COUNT(
				    	case when os.prescription_vision = 'progressive_rx'  AND os.payment = 'y' And os.status NOT IN ('return','cancelled','returned','failed','for exam', 'for payment', '' ) then 1 end
				    ) AS 'count_progressive_rx',
				    COUNT(
				    	case when os.product_upgrade = 'special_order'  AND os.payment = 'y' And os.status NOT IN ('return','cancelled','returned','failed','for exam', 'for payment', '' )  AND os.lens_code NOT IN ('SO003','SO002','SO001')  then 1 end
	...
	$query = 	"SELECT  
				...
				WHERE
					".$qDate."
						...
						".$removeStoreIDs."
						AND os.status IN ('complete', 'dispatched', 'paid', 'received')
						AND (os.product_upgrade = 'G100' OR os.product_code = 'C100')					
						AND os.payment = 'y'
						AND os.dispatch_type!='packaging'	
				GROUP BY
					os.product_code						
...
function grabBestFramesTableContactLens() {
...
function grabRevenueBreakdown() {
	...
	// Group Filter for separating between Offline Branches and ROVING / SHO		
	$arrGroupFilter = array('991', '995', '147', '148', '149');
	...
	for ($i=0; $i < sizeOf($arrRevenue); $i++) { 					
		...
		// Set current data - Store ID
		$curStoreID    = $arrRevenue[$i]['store_id'];
		$curStoreTotal = $arrRevenue[$i]['total'];

		if(!in_array($curStoreID, $arrGroupFilter)) {

			$groupATotal += $curStoreTotal;

		}
		else {

			$groupBTotal += $curStoreTotal;
	...
	// Header
	echo 		'<div class="custom-card-header row flex-column flex-md-row no-gutters align-items-md-center justify-content-md-between">';
	echo 			'<p class="h3 font-bold">Offline Branches</p>';
	echo 			'<p class="h3 font-bold">₱'.number_format($groupATotal, 2, '.', ',').'</p>';
	echo 		'</div>';

	// ECHO DATA LIST
	echo '<div class="row">
			<div class="col-5 col-md-3 mb-2 text-left headerStore">
				<p>Store</p>
			</div>
			<div class="col-2 col-md-1 mb-2 text-left headerStaff">
					<p>Staff</p>
			</div>
			<div class="col-2 col-md-1 mb-2 text-left headerDoctor">
					<p>Doctors</p>
			</div>
	...
	for ($i=0; $i < sizeOf($arrRevenue); $i++) { 					
		...
		if(!in_array($curStoreID, $arrGroupFilter)) {
			...
			$doctorCount   = grabDoctorsCount($arrRevenue[$i]['store_id']);		
			$storenumber   = $counterGroupA;
			...
			echo 			'<div class="row" style="height: 55px;">';
			echo 				'<p class="col-1 text-left topAdjust">'.$storenumber.'</p>';
			echo 				'<a class="col-7 col-lg-9 pr-0" style="text-decoration: underline !important;" target="_BLANK" href="/dashboard/philippines/?filterStores%5B%5D='.$curStoreID.'&'.$date_filter.'">';
			echo 					'<p class="text-left topAdjust">'.$curStoreName.'</p>';
			echo 				'</a>';
			echo 			'</div>';
			echo 		'</div>';
			echo 		'<div class="col-2 col-md-1  text-left">';
			echo 			'<div class="row flex-column flex-md-row no-gutters align-items-start align-items-md-center pl-1" style="border-left: 1px solid #000000; height:55px;">';
			echo 				'<div class="store-total">';
			echo 					'<p class="text-left pt-2 pb-2 pl-1">'.$empCount.'</p>';
			echo 				'</div>';
			echo 			'</div>';
			echo 		'</div>';
			...
			echo 			'<div class="row flex-column flex-md-row no-gutters align-items-start align-items-md-center pl-1" style="border-left: 1px solid #000000; height:55px;">';
			echo 				'<div class="store-total">';
			echo 					'<p class="text-left pt-2 pb-2 pl-1">'.$doctorCount.'</p>';
			echo 				'</div>';
			echo 			'</div>';
			echo 		'</div>';

			echo 		'<div class="col-3 col-md-7">';
			echo 			'<div class="row flex-column flex-md-row no-gutters align-items-start align-items-md-center pl-1" style="border-left: 1px solid #000000; height: 55px;">';
			echo 				'<div class="col-12 col-lg-2">';
			echo 					'<p class="col-12 text-left pt-2 pb-2 pl-1">₱'.( number_format($arrRevenue[$i]['total'], 2, '.', ',') ).'</p>';
			echo 				'</div>';
	...
	// Divider
	echo 	'<div style="padding: 30px 0;"></div>';

	// Header
	echo 		'<div class="custom-card-header row flex-column flex-md-row no-gutters align-items-md-center justify-content-md-between">';
	echo 			'<p class="h3 font-bold">Roving + SHO Branches</p>';
	echo 			'<p class="h3 font-bold">₱'.number_format($groupBTotal, 2, '.', ',').'</p>';
	echo 		'</div>';
...
function grabDoctorsCount($storeCode) {
	...
	$query = 	"SELECT DISTINCT
				...
				FROM
					...
					LEFT JOIN orders_specs os
						ON os.order_id = o.order_id
				WHERE ".$qDate."
				AND o.origin_branch = '".$storeCode."'
				AND o.doctor !=''
				AND os.status IN ('complete', 'dispatched', 'paid', 'received')
				AND os.order_id IS NOT NULL
				AND os.dispatch_type!='packaging'
				ORDER BY DATE(os.payment_date);";
	// echo $query; exit;
	$grabParams = array(
		'total',
		'payment_date'
	...
	if (mysqli_stmt_prepare($stmt, $query)) {
		...
		while (mysqli_stmt_fetch($stmt)) {
			...
			if (array_key_exists($tempArray['payment_date'],$arrTotal)){
				$arrTotal[$tempArray['payment_date']] += 1;
			}else{
				$arrTotal[$tempArray['payment_date']] = 1;
			}
...
function grabStores() {
	...
	$specStore = $removeStoreLocations;
	if($_SESSION['user_login']['position'] == 'supervisor'){
	// Set WHERE query for stores
		$arrStoreSupervisor  =  explode(',', $_SESSION['user_login']['store_location']);

		$specStore = " sl.store_id IN (";

		for ($i=0; $i < sizeOf($arrStoreSupervisor); $i++) { 
		
			$specStore .= "'".$arrStoreSupervisor[$i]."'";

			if($i < sizeOf($arrStoreSupervisor) - 1) {
...
function grabGuestTable(){
	...
	$query ="SELECT 
					...
					FROM orders o 
						...
						WHERE
							...
							AND os.po_number!=''
							AND os.orders_specs_id!=''
							AND pi.email_address LIKE '%specsguest@sunniesspecsoptical.com%'
	...
	for ($i=0; $i < sizeOf($arrGuest); $i++) { 
		echo 	'<tr class="row100 body">';
		echo 		'<td nowrap class="cell100 small text-center">'.($i + 1).'</td>';					
		echo 		'<td nowrap class="cell100 small">'.$arrGuest[$i]['item_name'].'</td>';
		echo 		'<td nowrap class="cell100 small">'.$arrGuest[$i]['count'].'</td>';
		echo 		'<td nowrap class="cell100 small">'.number_format($arrGuest[$i]['total'],2).'</td>';
		echo 		'<td nowrap class="cell100 small">'.number_format($arrGuest[$i]['total']*$arrGuest[$i]['count'],2).'</td>';			
		echo 	'</tr>';
	};
}
...
if(isset($_GET['function'])) {
	...
	switch ($_GET['function']) {
		...
		case 'grabBestFramesTableContactLens':
			grabBestFramesTableContactLens();
			break;
