...
require $sDocRoot."/includes/dashboard_v3/functions.php";
...
// Grab all the services orders
$arrRevenueServices = grabRevenueServices();	
...
for ($i=0; $i < sizeOf($arrRevenueServices); $i++) { 

	// Set current data
	$curDay 	  = $arrRevenueServices[$i]['day'];
	$curOrderDate = $arrRevenueServices[$i]['order_date'];
	$curTotal 	  = $arrRevenueServices[$i]['total'];	

	$grandTotalServices += $curTotal;

};

echo '<p id="services-revenue-grand-total" data-grand-total="'.( number_format($grandTotalServices, 2, '.', ',')  ).'" data-grand-total-raw="'.$grandTotalServices.'" style="display: none;"></p>';
...
if(isset($_GET['date']) && $_GET['date'] == 'custom') {
	...
	$curMonth = date("F", strtotime($_GET["data_range_start_year"]."-".$_GET["data_range_start_month"]."-".$_GET["data_range_start_day"]));
	$curMonthNum = date("Y-m", strtotime($_GET["data_range_start_year"]."-".$_GET["data_range_start_month"]."-".$_GET["data_range_start_day"]));

	for ($i=0; $i < $arrMonthDays[$curMonth]; $i++) { 
	
		$arrMonth[sprintf("%02d", ($i + 1))] = array();		

	};

	for ($i=0; $i < sizeOf($arrRevenue); $i++) { 
	
		$curRevDay 		 = $arrRevenue[$i]['day'];

		$arrMonth[$curRevDay]['day'] = $curRevDay;
		$arrMonth[$curRevDay]['order_date'] = $arrRevenue[$i]['order_date'];
		$arrMonth[$curRevDay]['subtotal'] = $arrRevenue[$i]['subtotal'];
		$arrMonth[$curRevDay]['total'] = $arrRevenue[$i]['total'];
		$arrMonth[$curRevDay]['count'] = $arrRevenue[$i]['count'];
	...
	if(isset($_GET['date'])) {
		...
		switch ($_GET['date']) {
			case 'monthYear':
				$curMonth = date("F", strtotime($_GET["data_range_year"]."-".$_GET["data_range_month"]."-01"));
				$curMonthNum = date("Y-m", strtotime($_GET["data_range_year"]."-".$_GET["data_range_month"]."-01"));
					for ($i=0; $i < $arrMonthDays[$curMonth]; $i++) { 
			...
			case 'pmonth':
				...
				$curMonthNum = date('Y-m', strtotime("-1 months"));
				break;
			case 'month':
				$curMonth 	= date("F");
				$curMonthNum = date('Y-m');
				break;
			}
		}else{
			$curMonth 	= date("F");
...
// Cycle through month days
 if(isset($_GET['date']) && $_GET['date'] == 'custom') {
 	...
 	$arrDaySelected = [];
 	$arrCustomDate = [];
 	$filteredDateFrom = $_GET["data_range_start_year"]."-".$_GET["data_range_start_month"]."-".$_GET["data_range_start_day"];
 	$filteredDateTo = $_GET["data_range_end_year"]."-".$_GET["data_range_end_month"]."-".$_GET["data_range_end_day"];

 	while(strtotime($filteredDateFrom) <= strtotime($filteredDateTo)){

 		$tempDate = strtotime($filteredDateFrom);
 		$arrCustomDate[] = $tempDate;
 		$arrDaySelected[] = date('d', $tempDate);
	 	$filteredDateFrom = date('Y-m-d',strtotime("+1 day",$tempDate ));
	 }

 	for ($i=0; $i < count($arrDaySelected); $i++) { 

 		$dayValue = 0;
 		foreach ($arrRevenue as $key => $value) {
 			if($value['day'] == $arrDaySelected[$i]){
 				$dayValue = $value['total'];
 				break;
 			}
 		}
		$day = date('D', $arrCustomDate[$i]);

		$day = $day[0];

		echo 			'["'.$arrDaySelected[$i].'\n'.$day.'", '.$dayValue.']';
		echo 		',';
	};
 }else{
	for ($i=0; $i < $arrMonthDays[$curMonth]; $i++) { 
		...
		$numDayText = ($numDay < 10) ? '0'.$numDay : $numDay;
		$day = date('D', strtotime($curMonthNum.'-'.$numDayText));
...
for ($i=0; $i < sizeOf($arrMonth); $i++) { 

	$numDay = ($i + 1);
	$numDayID = sprintf("%02d", $numDay);

	// Segment the revenue
	if($i < 7 && !empty($arrMonth[$numDayID]['total'])) {

		$arrWeeks[0] += $arrMonth[$numDayID]['total'];

	}
	elseif($i > 6 && $i < 14 && !empty($arrMonth[$numDayID]['total'])) {

		$arrWeeks[1] += $arrMonth[$numDayID]['total'];

	}
	elseif($i > 13 && $i < 21 && !empty($arrMonth[$numDayID]['total'])) {

		$arrWeeks[2] += $arrMonth[$numDayID]['total'];
		
	}
	else {
		if(isset($arrMonth[$numDayID]['total'])){
			$arrWeeks[3] += $arrMonth[$numDayID]['total'];
		}
