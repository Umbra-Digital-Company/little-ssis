...
if(isset($_GET['date']) && $_GET['date'] != 'custom') {
	...
	switch ($_GET['date']) {
		...
		case 'yesterday':		
			...
			$qDate = 	"DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%d') = ".$qGrabDateA." 
							AND DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%m') = ".$qGrabDateB."
							AND DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%Y') = ".$qGrabDateC."
							AND DATE_FORMAT(os.payment_date, '%d') = ".$qGrabDateA." 
		...
		case 'week':
			...
			$qDate = 	"YEARWEEK(DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%Y-%m-%d'), 1) = YEARWEEK('".$qGrabDateA."', 1)
							AND YEARWEEK(DATE_FORMAT(os.payment_date, '%Y-%m-%d'), 1) = YEARWEEK('".$qGrabDateA."', 1)";
			break;

		case 'month':
			$qGrabDateA = date("m");
			$qGrabDateB = date("Y");
			$qDate = 	"DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%m') = ".$qGrabDateA."
							AND DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%Y') = ".$qGrabDateB."
							AND DATE_FORMAT(os.payment_date, '%m') = ".$qGrabDateA."
							AND DATE_FORMAT(os.payment_date, '%Y') = ".$qGrabDateB;
			break;
		
		case 'year':
			$qGrabDate = date("Y");
			$qDate = "DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%Y') = ".$qGrabDate."
						AND DATE_FORMAT(os.payment_date, '%Y') = ".$qGrabDate;
			break;
...
elseif(isset($_GET['data_range_start_month'])) {
	...
	$qDateA = 	"DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%Y-%m-%d') >= '".$dateStart."'
					AND DATE_FORMAT(os.payment_date, '%Y-%m-%d') >= '".$dateStart."'";
	...
	if(isset($_GET['data_range_end_month'])) {
		...
		$qDateB = " AND DATE_FORMAT(DATE_ADD(DATE_FORMAT(o.date_created, '%Y-%m-%d %H:%i'), INTERVAL 12 Hour), '%Y-%m-%d') <= '".$dateEnd."'
					AND DATE_FORMAT(os.payment_date, '%Y-%m-%d') <= '".$dateEnd."'";
...
function grabRevenue($option_return = NULL) {
	...
	if($option_return != NULL) {
		...
		echo 		'<li id="revenue-grand-total" data-grand-total="'.( number_format($grandTotal, 2, '.', ',')  ).'"></li>';
		echo 	'</ul>';
