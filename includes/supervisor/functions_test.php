...
if(isset($_GET['filterStores'])) {
	...
	// Check to ensure stores are allowed
	for ($i=0; $i < sizeOf($arrFilterStores); $i++) { 
	
		// Set current store
		$curStoreCheck = $arrFilterStores[$i];

		// Reset everything if store is not in Session Stores
		if(!in_array($curStoreCheck, $arrSessionStores)) {

			// Error Order ID Checkup
		    $to = "ericdeejr@sunniesstudios.com, michelhodge@umbradigitalcompany.com, nylangeles@umbradigitalcompany.com";
		    $subject = "Supervisor - Supscious Activity";
		    $message =  "<html>
		                    <head>
		                        <title>Supervisor - Suspicious Activity</title>
		                    </head>
		                    <body>
		                        <p>
		                        	Hi Eric,
		                        	<br>
		                        	<br>
		                        	Suspicious activity was detected in <b>".$_SESSION['user_login']['username']."</b>'s account.
		                        	<br>
		                        	<br>
		                        	The following Store ID was searched for: <b>".$curStoreCheck."</b>
		                        	<br>
		                        	<br>
		                        	Sincerely,
		                        	<br>
		    ...
		    // More headers
		    $headers .= 'From: <webmaster@sunniessystems.com>' . "\r\n";

		    mail($to,$subject,$message,$headers);

			header('Location: /dashboard/supervisor/');			
			exit;
...
function grabRevenueBreakdown() {
	...
	global $arrFilterStores;
	global $removeInternational;
	global $removeInternationalStores;
