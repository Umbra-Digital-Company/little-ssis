...
$query =    "SELECT
            ...
            FROM 
                poll_51 p51
                    ...
                    LEFT JOIN the_archive_specs arch
...
for ($i=0; $i < sizeOf($arrSpecs); $i++) { 
    ...
    $curColor        = $arrSpecs[$i]['general_color_name'];
    $curColorCode    = $arrSpecs[$i]['general_color_code'];
    $curShape        = $arrSpecs[$i]['shapes_name'];
    $curShapeCode    = $arrSpecs[$i]['shapes_code'];
    $curWidth        = $arrSpecs[$i]['width_name'];
    $curWidthCode    = $arrSpecs[$i]['width_code'];
    $curMaterial     = $arrSpecs[$i]['material_name'];
    $curMaterialCode = $arrSpecs[$i]['material_code'];
    $curFinish       = $arrSpecs[$i]['finish_name'];
    $curFinishCode   = $arrSpecs[$i]['finish_code'];
