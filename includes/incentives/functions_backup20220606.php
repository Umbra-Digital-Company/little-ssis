	...
	foreach ($arrIncentives as $keyRow => $line) {
		...
		if($valueThreshold != 'n'){
			if($sramDataBelow > 0){
				...
				$line['amBT'] = $negative.($multiplierValueBelow * $amDataBelow);
				$line['supareaBT'] = $negative.($multiplierValueBelow * $supareaDataBelow);
			...
			if($sramDataAbove > 0 && $line['excess'] >= 0){
				...
				$line['amAT'] = $negative.($line['excess'] * $amDataAbove);
				$line['supareaAT'] = $negative.($line['excess'] * $supareaDataAbove);
...
function grabIncentivesCommissionBreakdownTable() {
	...
	foreach ($arrStaffIncentives as $key => $value) {
			...
			foreach ($value['dates'] as $keyDate => $valueDate) {
				...
				echo 		'<td nowrap class="cell100 small">'.$keyDate.'</td>';
				echo 	'</tr>';
				$rowN++;
			}
	}
