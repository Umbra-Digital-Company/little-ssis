...
switch ($page) {
	...
	case 'dispatch-studios':
	case 'dispatch-customer':
		$pDispatch = 'active';
		$nav_return = '/dashboard/';
		$nav_specific_return = '/studios/dispatch-studios/';
		$nav_inventory = '/studios/inventory/store/studios-receive/';
	...
	case 'appointment-attach':
			...
			$nav_return = '/dashboard/';
			$nav_specific_return = '/studios/dispatch-studios/';
			$nav_inventory = '/inventory/store/studios-receive/';
			$nav_appointment = '/appointment-attach/';
			$online_order = '/online-orders/';
			break;
...
if($user_type==1){
		$nameSis = ($_SESSION['store_type'] == 'ds' || $_SESSION['store_type'] == 'sr') ? 'SIS Mini' : 'Little SIS';
	$lil_ssis="<div class='col-md-6 col-xs-12 mb-3'><a href='https://sunniessystems.com/sis/studios/v1.0/?page=store-home'><button class='btn btn-primary' style='height: 100px; width: 100%;'>".$nameSis."</button></a></div>";
...
if($_SESSION['store_code']!='142' &&  $_SESSION['store_code']!='150' && $_SESSION['store_code']!='155' && $_SESSION['store_code']!='787'   && $_SESSION['store_code']!='788'){
	...
	if($_SESSION['store_type'] != 'ds' && $_SESSION['store_type'] != 'sr'){
...
$navbar_return = 	'<nav class="row no-gutters justify-content-between">
							...
							<div class="col-6 text-right justify-content-between">	
								<div class="nav-link dropdown-toggle d-inline btn-spoiler" type="button" data-toggle="collapse" data-target="#tabletMenu" aria-expanded="false" aria-controls="tabletMenu" style="padding-right: 0; font-size: 13px;">DISPATCH MENU</div>	
							</div>
							<div class="col-12 mt-3 collapse" id="tabletMenu">
								<div class="card card-body" style="background: #e6e6e6;">
									<div class="row text-center justify-content-between">	
										...
										';
// if(isset($_SESSION['user_access']['aim_studios']['receive_store_studios']) && $_SESSION['user_access']['aim_studios']['receive_store_studios']){
// 	$navbar_return	.=					'<div class="col-md-6 col-xs-12 mb-3">
...
$navbar_return_b = 	'<nav class="row no-gutters align-items-center justify-content-between">
						...
						'.$promo_code_checker.' 
						'.$code_connect.' 		
						<a class="nav-link d-inline" href="'.$nav_specific_return.'">Dispatch Sun</a>	
							'.$lil_ssis.'				
					...
					<div class="col-12 mt-3 collapse" id="tabletMenu">
						<div class="card card-body" style="background: #e6e6e6;">
							<div class="row text-center justify-content-between">	
									...
									'.$dispatchBrandLink.'	
								<div class="col-md-6 col-xs-12 mb-3">
									<a href="'.$nav_specific_return.'"><button class="btn btn-primary" style="height: 100px; width: 100%;">Dispatch Sun</button></a>	
								</div>';
if(isset($_SESSION['user_access']['aim_studios']['receive_store_studios']) && $_SESSION['user_access']['aim_studios']['receive_store_studios']){
	$navbar	.=			'<div class="col-md-6 col-xs-12 mb-3">
									<a href="'.$nav_inventory.'"><button class="btn btn-info" style="height: 100px; width: 100%;">Inventory</button></a>
								</div>';
}
