...
if(isset($_GET['filterStores'])) {
	...
	$arrFilterStores = array('147','148','149','787');
...
function grabFrames($number_of_frames = NULL) {
	...
	$query = 	"SELECT  
				...
				WHERE					
					...
					".$specStore."
					".$removeStoreIDs."
					AND (o.promo_code_type = 'affiliate' OR 1 = coalesce((SELECT IF(pc.type='affiliate',1,0) AS type FROM promo_codes pc WHERE  pc.promo_code = o.promo_code),'0'))
...
function grabPromoCodesTable() {
	...
	$query = 	"SELECT DISTINCT
					...
					WHERE
						...
						AND os.dispatch_type!='packaging'
						AND os.product_upgrade != 'SOW-500DP'
						AND (o.promo_code_type = 'affiliate' OR 1 = coalesce((SELECT IF(pc.type='affiliate',1,0) AS type FROM promo_codes pc WHERE  pc.promo_code = o.promo_code),'0')) ORDER BY o.promo_code;";
	$grabParams = array(
		'promo_code'
	);
	...
	if (mysqli_stmt_prepare($stmt, $query)) {
		...
		while (mysqli_stmt_fetch($stmt)) {
			...
			$arrPromoCodes[] = $tempArray['promo_code'];
	...
	for($a = 0; $a < sizeof($arrPromoCodes); $a++){

		// Set array
		$arrPromoCodesData = array();
		...
		$query = 	"SELECT
						...
						WHERE
							...
							AND os.product_upgrade != 'SOW-500DP'
							AND (o.promo_code_type = 'affiliate' OR 1 = coalesce((SELECT IF(pc.type='affiliate',1,0) AS type FROM promo_codes pc WHERE  pc.promo_code = o.promo_code),'0'))
							AND o.promo_code = '".$arrPromoCodes[$a]."'
		...
		if (mysqli_stmt_prepare($stmt, $query)) {
			...
			while (mysqli_stmt_fetch($stmt)) {
				...
				$arrPromoCodesData = [
					'amount_order' => $tempArray['amount_order'],
					'sum_order_amount' => $tempArray['sum_order_amount']
				];
		...
		echo 		'<td nowrap class="cell100 small text-center">'.($a + 1).'</td>';					
		echo 		'<td nowrap class="cell100 small">'.$arrPromoCodes[$a].'</td>';
		echo 		'<td nowrap class="cell100 small">'.$arrPromoCodesData['amount_order'].'</td>';
		echo 		'<td nowrap class="cell100 small">'.number_format($arrPromoCodesData['sum_order_amount'],2).'</td>';
		echo 		'<td nowrap class="cell100 small">'.number_format(($arrPromoCodesData['sum_order_amount']*.1),2).'</td>';			
		echo 	'</tr>';
	}

};

function grabPromoCodesTableCSV() {
	...
	if (mysqli_stmt_prepare($stmt, $query)) {
		...
		while (mysqli_stmt_fetch($stmt)) {
			...
			$tempArray['po_number'] = '="'.$tempArray['po_number'].'"';
			$sum_amount = $tempArray['sum_order_amount'];
			$tempArray['sum_order_amount'] = number_format($sum_amount,2);
			$tempArray['commision'] = number_format(($sum_amount * .1),2);
			$arrPromoCodes[] = $tempArray;
	...
	$filename = "affiliate-code-breakdown-".date('YmdHis').".csv";
    ...
    fputcsv($handle, ['CODE','AMOUNT OF ORDERS','PO NUMBER','TOTAL ORDER AMOUNT', '10% COMMSION']);

    foreach ($arrPromoCodes as $key => $line) {
    	fputcsv($handle, $line);
    }
...
function grabPerformingCodes() {
	...
	$query = 	"SELECT
					o.promo_code,
					COUNT(o.promo_code) AS 'count_promo'
					FROM orders o
					WHERE
						o.order_id IN (SELECT DISTINCT os.order_id FROM orders_specs os WHERE ".$qDate."
						AND os.payment = 'y'
						AND os.status IN ('complete', 'dispatched', 'paid', 'received')
						AND os.dispatch_type!='packaging'
						AND os.product_upgrade != 'SOW-500DP')
						...
						".$removeStoreIDs."
						AND (o.promo_code_type = 'affiliate' OR 1 = coalesce((SELECT IF(pc.type='affiliate',1,0) AS type FROM promo_codes pc WHERE  pc.promo_code = o.promo_code),'0'))
						GROUP BY o.promo_code ORDER BY count_promo DESC LIMIT 5;";
	$grabParams = array(
		'promo_code',
		'count'
	);
...
function grabRevenueBreakdown() {
	...
	if((isset($_GET['date']) && $_GET['date'] != 'custom') || $customMonth) {
		...
		$query .= 	" AND (o.promo_code_type = 'affiliate' OR 1 = coalesce((SELECT IF(pc.type='affiliate',1,0) AS type FROM promo_codes pc WHERE  pc.promo_code = o.promo_code),'0')) AND os.status IN ('dispatched', 'received', 'paid', 'complete')
			AND os.dispatch_type!='packaging'
			AND os.product_upgrade!='SOW-500DP'
								GROUP BY
									store_id_order
...
if(isset($_GET['function'])) {
	...
	switch ($_GET['function']) {
		...
		case 'grabPromoCodesTable':
			grabPromoCodesTable();
			break;
		case 'grabPromoCodesTableCSV':
			grabPromoCodesTableCSV();
			break;
