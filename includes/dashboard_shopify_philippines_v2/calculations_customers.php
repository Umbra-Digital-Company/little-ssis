...
for ($i=0; $i < sizeOf($arrCustomersInfo); $i++) { 
	...
	// Set current data
	$curAge = $arrCustomersInfo[$i]['age_final'];
	if(!isset($arrAgeBreakdown[$curAge]['count'])){
		$arrAgeBreakdown[$curAge]['count'] = 1;
	}else{
...
for ($i=0; $i < sizeOf($arrCustomersInfo); $i++) { 
	...
	if($curCity != 'N/A' && $curCity != '' && $curCity != NULL) {		
		...
		if(!isset($arrLocationsBreakdown[$curCity]['count'])){
			$arrLocationsBreakdown[$curCity]['count'] = 1;
		}else{
	...
	elseif($curCity == 'N/A' && $curCountry != 'philippines') {
		...
		if(!isset($arrLocationsBreakdownINT[$curCountry]['count'])){
			$arrLocationsBreakdownINT[$curCountry]['count'] = 1;
		}else{
...
echo '<div class="loader" id="data-genders" data-male="'.$cMale.'" data-female="'.$cFemale.'"></div>';
echo '<div class="loader" id="data-customer-types" data-new-customer="'.$cNew.'" data-recurring-customer="'.$cRecurring.'"></div>';
