...
require $sDocRoot."/includes/dashboard_shopify_philippines_v2/functions.php";
...
for ($i=0; $i < sizeOf($arrTopFrames); $i++) { 
	...
	$totalFramesWithoutPRX +=$arrTopFrames[$i]['non-prescription'];


	// Prescription Types
	$totalFramesSVStock += $arrTopFrames[$i]['count_single_vision_stock'];
	$totalFramesSVStock += $arrTopFrames[$i]['single_vision_standard'];
...
$arrPRXTypes = array(
	...
	),"single_vision_standard" => array(

		"type" => "single_vision_standard",
		"count" => $totalFramesSVStock,
		"lenses" => array()
...
data-sum-housebrand="'.( $arrPRXTypes['house_brand']['sum'] ).'"></div>';
...
for ($i=0; $i < sizeOf($arrItemsMerch); $i++) { 
	...
	if($arrItemsMerch['item_name'] != NULL && $arrItemsMerch['item_name'] != '') {
		if(isset($arrItemsMerch['item_name'])){
...
for ($i=0; $i < sizeOf($arrItemsAntiRad); $i++) { 
	...
	// Set current data
	if($arrItemsAntiRad[$i]['item_name'] != NULL && $arrItemsAntiRad[$i]['item_name'] != '') {
...
if(isset($arrPRXTypes['single_vision_stock']) && !empty($arrPRXTypes['single_vision_stock'])) {
	...
	// Set specific array
	$arrSVS = array_merge(array_values($arrPRXTypes['single_vision_stock']['lenses']),array_values($arrPRXTypes['single_vision_standard']['lenses']));
	// Cycle through lenses array
	for ($i=0; $i < sizeOf($arrSVS); $i++) { 
		if($arrSVS[$i]['lens_code'] == 'SO1001') continue;
