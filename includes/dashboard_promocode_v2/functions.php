...
function grabFrames($number_of_frames = NULL) {
	...
	$query = 	"SELECT  
				...
				WHERE					
					...
					".$removeStoreIDs."
					AND o.promo_code != ''
					AND o.promo_code_type != 'affiliate' 
					AND 1 = coalesce((SELECT IF(pc.type='affiliate',0,1) AS type FROM promo_codes pc WHERE  pc.promo_code = o.promo_code),'1')
...
function grabPromoCodesTable() {
	...
	// arrange data sort by total order amount
	...
	for($a = 0; $a < sizeof($arrPromoCodes); $a++){
		...
		$arrSortPromoCode[] = [
			'promo_code' => $arrPromoCodes[$a],
			'amount_order' => $arrPromoCodesData['amount_order'],
			'sum_order_amount' => $arrPromoCodesData['sum_order_amount'],
			'ten_percent' => $arrPromoCodesData['sum_order_amount']*.1
		];
	}
	usort($arrSortPromoCode, function ($item1, $item2) {
	    return $item2['sum_order_amount'] <=> $item1['sum_order_amount'];
	});
	foreach ($arrSortPromoCode as $key => $value) {
		echo 	'<tr class="row100 body">';
		echo 		'<td nowrap class="cell100 small text-center">'.($key + 1).'</td>';					
		echo 		'<td nowrap class="cell100 small">'.$value['promo_code'].'</td>';
		echo 		'<td nowrap class="cell100 small">'.$value['amount_order'].'</td>';
		echo 		'<td nowrap class="cell100 small">'.number_format($value['sum_order_amount'],2).'</td>';			
		echo 	'</tr>';
	}
...
function grabPromoCodesTableCSV() {
	...
	$filename = "promocode-breakdown-".date('YmdHis').".csv";
    ...
    fputcsv($handle, ['CODE','AMOUNT OF ORDERS','PO NUMBER','TOTAL ORDER AMOUNT']);
...
function grabRevenueBreakdown() {
	...
	if((isset($_GET['date']) && $_GET['date'] != 'custom') || $customMonth) {
		...
		$query .= 	" AND o.promo_code != '' AND o.promo_code_type != 'affiliate' 
					AND 1 = coalesce((SELECT IF(pc.type='affiliate',0,1) AS type FROM promo_codes pc WHERE  pc.promo_code = o.promo_code),'1') AND os.status IN ('dispatched', 'received', 'paid', 'complete')
								GROUP BY
									store_id_order
							) AS q
