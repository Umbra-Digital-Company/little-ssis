...
$removePhilippines = "AND o.currency NOT IN ('php', 'usd')";
$removeOtherStores = "sl.country NOT IN ('philippines', 'rest-of-world')";
...
function grabItemsSoldSpecs() {
	...
	$query = 	"SELECT  
				    ...
				    COUNT(
				    	case when os.lens_option = 'with prescription' AND os.status != 'cancelled' then 1 end
				    ) AS 'count_with_prescription',
				    COUNT(
				    	case when os.product_upgrade = 'fashion_lens' AND os.status = 'paid' then 1 end
				    ) AS 'count_without_prescription',
				    COUNT(
				    	case when os.lens_option = 'lens only' AND os.status != 'cancelled' then 1 end
				    ) AS 'count_lens_only',
				    COUNT(
				    	case when os.prescription_vision = 'single_vision_stock' AND os.status != 'cancelled' then 1 end
				    ) AS 'count_single_vision_stock',
				    COUNT(
				    	case when os.prescription_vision = 'single_vision_rx' AND os.status != 'cancelled' then 1 end
				    ) AS 'count_single_vision_rx',
				    COUNT(
				    	case when os.prescription_vision = 'double_vision_stock' AND os.status != 'cancelled' then 1 end
				    ) AS 'count_double_vision_stock',
				    COUNT(
				    	case when os.prescription_vision = 'double_vision_rx' AND os.status != 'cancelled' then 1 end
				    ) AS 'count_double_vision_rx',
				    COUNT(
				    	case when os.prescription_vision = 'progressive_stock' AND os.status != 'cancelled' then 1 end
				    ) AS 'count_progressive_stock',
				    COUNT(
				    	case when os.prescription_vision = 'progressive_rx' AND os.status != 'cancelled' then 1 end
				    ) AS 'count_progressive_rx',
				    COUNT(
				    	case when os.product_upgrade = 'special_order' AND os.status != 'cancelled' then 1 end
...
function grabRevenueBreakdown() {
	...
	for ($i=0; $i < sizeOf($arrRevenue); $i++) { 
		...
		echo 				'<a class="col-9" style="text-decoration: underline !important;" target="_BLANK" href="/dashboard/vietnam/?filterStores%5B%5D='.$curStoreID.'&'.$date_filter.'">';
		echo 					'<p class="text-right">'.str_replace("Mega ", "", str_replace("Vietnam ", "", ucwords(strtolower($curStoreName)))).'</p>';
		echo 				'</a>';
		echo 			'</div>';		
		echo 		'</div>';
